...
import { KeybindingsConfig, KeybindingRule, ResolvedKeybindingRule, ResolvedKeybindingsConfig, } from "./keybindings";
const decode = (schema, input) => Schema.decodeUnknownEffect(schema)(input);
const decodeResolvedRule = Schema.decodeUnknownEffect(ResolvedKeybindingRule);
it.effect("parses keybinding rules", () => Effect.gen(function* () {
...
it.effect("rejects invalid command values", () => Effect.gen(function* () {
    const result = yield* Effect.exit(decode(KeybindingRule, {
        key: "mod+j",
        command: "script.Test.run",
    }));
    assert.strictEqual(result._tag, "Failure");
}));
it.effect("accepts dynamic script run commands", () => Effect.gen(function* () {
    const parsed = yield* decode(KeybindingRule, {
        key: "mod+r",
...
it.effect("parses keybindings array payload", () => Effect.gen(function* () {
...
it.effect("parses resolved keybinding rules", () => Effect.gen(function* () {
...
it.effect("parses resolved keybindings arrays", () => Effect.gen(function* () {
...
it.effect("drops unknown fields in resolved keybinding rules", () => decodeResolvedRule({
    command: "terminal.toggle",
    shortcut: {
        key: "j",
        metaKey: false,
