...
export function isTemporaryWorktreeBranch(branch) {
 ...
 */
export function parseGitHubRepositoryNameWithOwnerFromRemoteUrl(url) {
...
export function detectGitHostingProviderFromRemoteUrl(remoteUrl) {
...
export function mergeGitStatusParts(local, remote) {
    return {
        ...local,
        ...(remote ?? EMPTY_GIT_STATUS_REMOTE),
    };
}
function toRemoteStatusPart(status) {
    return {
...
function toLocalStatusPart(status) {
    return {
...
export function applyGitStatusStreamEvent(current, event) {
    switch (event._tag) {
        case "snapshot":
