...
export const makeDrainableWorker = (process) => Effect.gen(function* () {
    ...
    const takeItem = Effect.tx(Effect.gen(function* () {
        const item = yield* TxQueue.take(queue);
        yield* TxRef.update(ref, (n) => n + 1);
        return item;
    }));
    yield* takeItem.pipe(Effect.flatMap((item) => process(item).pipe(Effect.ensuring(TxRef.update(ref, (n) => n - 1)))), Effect.forever, Effect.forkScoped);
    const drain = Effect.tx(Effect.gen(function* () {
