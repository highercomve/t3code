...
const closeServer = (server) => Effect.sync(() => {
    try {
        server.close();
    }
    catch {
...
const openServer = (host) => Effect.callback((resume) => {
...
it.layer(NetService.layer)("NetService", (it) => {
    describe("Net helpers", () => {
        it.effect("reserveLoopbackPort returns a positive loopback port", () => Effect.gen(function* () {
            const net = yield* NetService;
            const port = yield* net.reserveLoopbackPort();
            assert.ok(port > 0);
        }));
        it.effect("isPortAvailableOnLoopback reports false for an occupied port", () => Effect.acquireUseRelease(openServer("127.0.0.1"), (server) => Effect.gen(function* () {
            const net = yield* NetService;
            const port = getPort(server);
            const available = yield* net.isPortAvailableOnLoopback(port);
            assert.equal(available, false);
        }), closeServer));
        it.effect("findAvailablePort returns preferred when it is free", () => Effect.gen(function* () {
            const net = yield* NetService;
        ...
        it.effect("findAvailablePort falls back when preferred is occupied", () => Effect.acquireUseRelease(openServer(), (server) => Effect.gen(function* () {
            const net = yield* NetService;
