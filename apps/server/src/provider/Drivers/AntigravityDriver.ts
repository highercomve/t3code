 ...
 *
 * Mirrors the Codex / Claude drivers: a plain value whose `create()` returns
 * one `ProviderInstance` bundling `snapshot` / `adapter` / `textGeneration`
 * closures captured over the per-instance `AntigravitySettings`.
 *
 * Antigravity is a one-shot, line-streaming provider (not ACP). Multi-turn
 * state is delegated to agy's own conversation store via `--conversation
 * <uuid>` and persisted by `AntigravityConversationStore`.
 *
