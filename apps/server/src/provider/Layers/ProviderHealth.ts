...
> = Effect.gen(function* () {
  ...
  // Probe: `gemini --version` — is the CLI reachable?
  ...
  // Gemini CLI auth happens during the ACP session handshake, not here.
  return {
    provider: GEMINI_PROVIDER,
    status: "ready" as const,
    available: true,
...
> = Effect.gen(function* () {
  ...
  // Probe: `opencode --version` — is the CLI reachable?
  ...
  // OpenCode auth happens during the ACP session handshake, not here.
  return {
    provider: OPENCODE_PROVIDER,
    status: "ready" as const,
    available: true,
