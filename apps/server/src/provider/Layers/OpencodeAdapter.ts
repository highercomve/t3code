 ...
 *
 * Wraps `OpencodeAppServerManager` behind the `OpencodeAdapter` service contract and
...
import {
  ...
  type OpencodeAppServerStartSessionInput,
...
function eventRawSource(event: ProviderEvent): NonNullable<ProviderRuntimeEvent["raw"]>["source"] {
  return event.kind === "request" ? "opencode.acp.request" : "opencode.acp.notification";
}
...
function mapToRuntimeEvents(
  ...
  // Note: gemini/event/* methods are Gemini-specific and not emitted by Opencode ACP.
...
const makeOpencodeAdapter = (options?: OpencodeAdapterLiveOptions) =>
  Effect.gen(function* () {
    ...
    const startSession: OpencodeAdapterShape["startSession"] = (input) => {
      ...
      const managerInput: OpencodeAppServerStartSessionInput = {
    ...
    const sendTurn: OpencodeAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        const opencodeAttachments = yield* Effect.forEach(
          input.attachments ?? [],
          (attachment) =>
            Effect.gen(function* () {
          ...
          try: () => {
            ...
            const managerInput = {
              ...
              ...(input.interactionMode !== undefined
                ...
                : {}),
              ...(opencodeAttachments.length > 0 ? { attachments: opencodeAttachments } : {}),
            };
    ...
    const readThread: OpencodeAdapterShape["readThread"] = (_threadId) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "readThread",
          detail: "readThread is not supported by the Opencode ACP provider.",
        }),
      );

    const rollbackThread: OpencodeAdapterShape["rollbackThread"] = (_threadId, _numTurns) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "rollbackThread",
          detail: "rollbackThread is not supported by the Opencode ACP provider.",
        }),
      );
        ...
        const listener = (event: ProviderEvent) =>
          Effect.gen(function* () {
            ...
            if (runtimeEvents.length === 0) {
              yield* Effect.logDebug("ignoring unhandled Opencode provider event", {
                method: event.method,
