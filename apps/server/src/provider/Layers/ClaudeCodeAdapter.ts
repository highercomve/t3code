...
function eventRawSource(event: ProviderEvent): NonNullable<ProviderRuntimeEvent["raw"]>["source"] {
  return event.kind === "request" ? "claudeCode.acp.request" : "claudeCode.acp.notification";
}
...
function mapToRuntimeEvents(
  ...
  // Note: gemini/event/* methods are Gemini-specific and not emitted by Claude Code ACP.
...
const makeClaudeCodeAdapter = (options?: ClaudeCodeAdapterLiveOptions) =>
  Effect.gen(function* () {
    ...
    const sendTurn: ClaudeCodeAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        const claudeCodeAttachments = yield* Effect.forEach(
          input.attachments ?? [],
          (attachment) =>
            Effect.gen(function* () {
        ...
        return yield* Effect.tryPromise({
          try: () => {
            const managerInput = {
              ...
              ...(input.modelOptions?.claudeCode?.reasoningEffort !== undefined
                ? { effort: input.modelOptions.claudeCode.reasoningEffort }
                : {}),
              ...(input.interactionMode !== undefined
                ? { interactionMode: input.interactionMode }
                : {}),
              ...(claudeCodeAttachments.length > 0 ? { attachments: claudeCodeAttachments } : {}),
            };
    ...
    const readThread: ClaudeCodeAdapterShape["readThread"] = (_threadId) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "readThread",
          detail: "readThread is not supported by the Claude Code ACP provider.",
        }),
      );

    const rollbackThread: ClaudeCodeAdapterShape["rollbackThread"] = (_threadId, _numTurns) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "rollbackThread",
          detail: "rollbackThread is not supported by the Claude Code ACP provider.",
        }),
      );
