...
export interface GeminiAdapterLiveOptions {
  ...
  readonly makeManager?: (services?: Context.Context<never>) => GeminiAppServerManager;
...
function geminiEventBase(
  ...
  return {
    ...
    ...(itemId ? { itemId: asRuntimeItemId(ProviderItemId.make(itemId)) } : {}),
...
const makeGeminiAdapter = (options?: GeminiAdapterLiveOptions) =>
  Effect.gen(function* () {
    ...
    const readThread: GeminiAdapterShape["readThread"] = (_threadId) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "readThread",
          detail: "readThread is not supported by the Gemini ACP provider.",
        }),
      );

    const rollbackThread: GeminiAdapterShape["rollbackThread"] = (_threadId, _numTurns) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "rollbackThread",
          detail: "rollbackThread is not supported by the Gemini ACP provider.",
        }),
      );
