import { OrchestrationGetFullThreadDiffInput, OrchestrationGetTurnDiffInput, } from "@t3tools/contracts";
...
function decodeCheckpointDiffRequest(input) {
    if (input.fromTurnCount === 0) {
        return Schema.decodeUnknownOption(OrchestrationGetFullThreadDiffInput)({
            threadId: input.threadId,
            toTurnCount: input.toTurnCount,
        }).pipe(Option.map((fields) => ({ kind: "fullThreadDiff", input: fields })));
    }
    return Schema.decodeUnknownOption(OrchestrationGetTurnDiffInput)({
        threadId: input.threadId,
        fromTurnCount: input.fromTurnCount,
        toTurnCount: input.toTurnCount,
    }).pipe(Option.map((fields) => ({ kind: "turnDiff", input: fields })));
}
function asCheckpointErrorMessage(error) {
...
function normalizeCheckpointErrorMessage(error) {
    ...
    if (lower.includes("checkpoint unavailable for thread") ||
...
export function checkpointDiffQueryOptions(input) {
    ...
    return queryOptions({
        ...
        retryDelay: (attempt, error) => isCheckpointTemporarilyUnavailable(error)
