...
vi.mock("../environments/runtime/service", () => ({
  ...
  subscribeEnvironmentConnections: (listener) => {
...
const TARGET = { environmentId: ENVIRONMENT_ID, cwd: "/repo" };
const FRESH_TARGET = { environmentId: ENVIRONMENT_ID, cwd: "/fresh" };
const BASE_STATUS = {
  isRepo: true,
...
function createRegisteredGitStatusClient(environmentId) {
  ...
  const client = {
    ...
    git: {
      ...
      onStatus: vi.fn((_, listener) => registerListener(listeners, listener)),
...
describe("gitStatusState", () => {
  ...
  it("keeps git status subscriptions isolated by environment when cwds match", () => {
    ...
    const localClient = {
      refreshStatus: vi.fn(),
      onStatus: vi.fn((_, listener) => registerListener(localListeners, listener)),
    };
    const remoteClient = {
      refreshStatus: vi.fn(),
      onStatus: vi.fn((_, listener) => registerListener(remoteListeners, listener)),
    };
    const remoteTarget = { environmentId: OTHER_ENVIRONMENT_ID, cwd: "/repo" };
