...
import { gitBranchSearchInfiniteQueryOptions, gitMutationKeys, gitPreparePullRequestThreadMutationOptions, gitPullMutationOptions, gitRunStackedActionMutationOptions, invalidateGitQueries, } from "./gitReactQuery";
...
const BRANCH_SEARCH_RESULT = {
    pages: [BRANCH_QUERY_RESULT],
    pageParams: [0],
};
describe("gitMutationKeys", () => {
    it("scopes stacked action keys by cwd", () => {
        expect(gitMutationKeys.runStackedAction("/repo/a")).not.toEqual(gitMutationKeys.runStackedAction("/repo/b"));
    });
    ...
    it("scopes pull request thread preparation keys by cwd", () => {
        expect(gitMutationKeys.preparePullRequestThread("/repo/a")).not.toEqual(gitMutationKeys.preparePullRequestThread("/repo/b"));
    });
});
...
describe("invalidateGitQueries", () => {
    it("can invalidate a single cwd without blasting other git query scopes", async () => {
        const queryClient = new QueryClient();
        queryClient.setQueryData(gitBranchSearchInfiniteQueryOptions({
            cwd: "/repo/a",
            query: "feature",
        }).queryKey, BRANCH_SEARCH_RESULT);
        queryClient.setQueryData(gitBranchSearchInfiniteQueryOptions({
            cwd: "/repo/b",
            query: "feature",
        }).queryKey, BRANCH_SEARCH_RESULT);
        await invalidateGitQueries(queryClient, { cwd: "/repo/a" });
        expect(queryClient.getQueryState(gitBranchSearchInfiniteQueryOptions({
            cwd: "/repo/a",
            query: "feature",
        }).queryKey)?.isInvalidated).toBe(true);
        expect(queryClient.getQueryState(gitBranchSearchInfiniteQueryOptions({
            cwd: "/repo/b",
            query: "feature",
        }).queryKey)?.isInvalidated).toBe(false);
    });
});
