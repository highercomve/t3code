...
function mapProjectsFromReadModel(incoming, previous) {
        ...
        return {
            ...
            defaultModelSelection: existing?.defaultModelSelection ??
                (project.defaultModelSelection
                    ? {
                        ...project.defaultModelSelection,
                        model: resolveModelSlugForProvider(project.defaultModelSelection.provider, project.defaultModelSelection.model),
                    }
                    : null),
    ...
    return mappedProjects
        ...
        const orderIndex = previousIndex ??
            persistedIndex ??
...
export function syncServerReadModel(state, readModel) {
    const projects = mapProjectsFromReadModel(readModel.projects.filter((project) => project.deletedAt === null), state.projects);
    ...
    const threads = readModel.threads
        ...
        return {
            ...
            modelSelection: {
                ...
                model: resolveModelSlugForProvider(thread.modelSelection.provider, thread.modelSelection.model),
            },
...
export function markThreadVisited(state, threadId, visitedAt) {
    ...
    const threads = updateThread(state.threads, threadId, (thread) => {
        ...
        if (Number.isFinite(previousVisitedAtMs) &&
...
export function setProjectExpanded(state, projectId, expanded) {
    ...
    const projects = state.projects.map((p) => {
        if (p.id !== projectId || p.expanded === expanded)
            return p;
        changed = true;
        return { ...p, expanded };
    });
...
export const useStore = create((set) => ({
    ...
    reorderProjects: (draggedProjectId, targetProjectId) => set((state) => reorderProjects(state, draggedProjectId, targetProjectId)),
