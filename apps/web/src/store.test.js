...
function withActiveEnvironmentState(environmentState, overrides = {}) {
  const {
...
function makeState(thread) {
  ...
  const environmentState = {
    ...
    messageByThreadId: {
      [thread.id]: Object.fromEntries(thread.messages.map((message) => [message.id, message])),
    },
    activityIdsByThreadId: {
      [thread.id]: thread.activities.map((activity) => activity.id),
    },
    activityByThreadId: {
      [thread.id]: Object.fromEntries(thread.activities.map((activity) => [activity.id, activity])),
    },
    proposedPlanIdsByThreadId: {
      [thread.id]: thread.proposedPlans.map((plan) => plan.id),
    },
    proposedPlanByThreadId: {
      [thread.id]: Object.fromEntries(thread.proposedPlans.map((plan) => [plan.id, plan])),
    },
    ...
    turnDiffSummaryByThreadId: {
      [thread.id]: Object.fromEntries(
        thread.turnDiffSummaries.map((summary) => [summary.turnId, summary]),
      ),
    },
...
function localEnvironmentStateOf(state) {
  return selectEnvironmentState(state, localEnvironmentId);
}
function environmentStateOf(state, environmentId) {
  return selectEnvironmentState(state, environmentId);
}
function projectsOf(state) {
