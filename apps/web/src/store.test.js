import { CheckpointRef, DEFAULT_MODEL_BY_PROVIDER, EventId, MessageId, ProjectId, ThreadId, TurnId, } from "@t3tools/contracts";
import { describe, expect, it } from "vitest";
import { applyOrchestrationEvent, applyOrchestrationEvents, syncServerReadModel, } from "./store";
...
function makeEvent(type, payload, overrides = {}) {
    ...
    return {
        ...
        aggregateId: "threadId" in payload
            ? payload.threadId
...
function makeReadModelThread(overrides) {
    return {
        id: ThreadId.makeUnsafe("thread-1"),
        projectId: ProjectId.makeUnsafe("project-1"),
        title: "Thread",
...
function makeReadModelProject(overrides) {
    return {
        id: ProjectId.makeUnsafe("project-1"),
        title: "Project",
        workspaceRoot: "/tmp/project",
...
describe("incremental orchestration updates", () => {
    it("does not mark bootstrap complete for incremental events", () => {
        ...
        const next = applyOrchestrationEvent(state, makeEvent("thread.meta-updated", {
            threadId: ThreadId.makeUnsafe("thread-1"),
            title: "Updated title",
    ...
    it("updates the existing project title when project.meta-updated arrives", () => {
        ...
        const next = applyOrchestrationEvent(state, makeEvent("project.meta-updated", {
            projectId,
    ...
    it("preserves state identity for no-op project and thread deletes", () => {
        ...
        const nextAfterProjectDelete = applyOrchestrationEvent(state, makeEvent("project.deleted", {
            projectId: ProjectId.makeUnsafe("project-missing"),
            deletedAt: "2026-02-27T00:00:01.000Z",
        }));
        const nextAfterThreadDelete = applyOrchestrationEvent(state, makeEvent("thread.deleted", {
    ...
    it("reuses an existing project row when project.created arrives with a new id for the same cwd", () => {
        ...
        const next = applyOrchestrationEvent(state, makeEvent("project.created", {
    ...
    it("removes stale project index entries when thread.created recreates a thread under a new project", () => {
        ...
        const next = applyOrchestrationEvent(state, makeEvent("thread.created", {
            threadId,
            projectId: recreatedProjectId,
    ...
    it("updates only the affected thread for message events", () => {
        ...
        const next = applyOrchestrationEvent(state, makeEvent("thread.message-sent", {
            threadId: thread1.id,
    ...
    it("does not regress latestTurn when an older turn diff completes late", () => {
        ...
        const next = applyOrchestrationEvent(state, makeEvent("thread.turn-diff-completed", {
            threadId: ThreadId.makeUnsafe("thread-1"),
            turnId: TurnId.makeUnsafe("turn-1"),
    ...
    it("rebinds live turn diffs to the authoritative assistant message when it arrives later", () => {
        ...
        expect(next.threads[0]?.turnDiffSummaries[0]?.assistantMessageId).toBe(MessageId.makeUnsafe("assistant-real"));
        expect(next.threads[0]?.latestTurn?.assistantMessageId).toBe(MessageId.makeUnsafe("assistant-real"));
    });
    it("reverts messages, plans, activities, and checkpoints by retained turns", () => {
        ...
        const next = applyOrchestrationEvent(state, makeEvent("thread.reverted", {
            threadId: ThreadId.makeUnsafe("thread-1"),
            turnCount: 1,
        }));
    ...
    it("clears pending source proposed plans after revert before a new session-set event", () => {
        ...
        const reverted = applyOrchestrationEvent(makeState(thread), makeEvent("thread.reverted", {
            threadId: thread.id,
            turnCount: 1,
        }));
        expect(reverted.threads[0]?.pendingSourceProposedPlan).toBeUndefined();
        const next = applyOrchestrationEvent(reverted, makeEvent("thread.session-set", {
            threadId: thread.id,
            session: {
                threadId: thread.id,
                status: "running",
