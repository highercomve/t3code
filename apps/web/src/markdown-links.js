...
function parseFileUrlHref(href, options) {
  try {
...
export function rewriteMarkdownFileUriHref(href) {
  if (!href) return null;
  const target = parseFileUrlHref(href.trim(), { decodePath: false });
  if (!target) return null;
  return `${target.path}${target.hash}`;
}
function looksLikePosixFilesystemPath(path) {
...
function appendLineColumnFromHash(path, hash) {
...
function isLikelyPathCandidate(path) {
...
function hasExternalScheme(path) {
...
export function resolveMarkdownFileLinkTarget(href, cwd) {
  if (!href) return null;
