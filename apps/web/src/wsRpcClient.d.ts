import { type GitActionProgressEvent, type GitRunStackedActionInput, type GitRunStackedActionResult, type GitStatusResult, type GitSuggestCommitMessageInput, type GitSuggestCommitMessageResult, type NativeApi, ORCHESTRATION_WS_METHODS, type ServerSettingsPatch, WS_METHODS } from "@t3tools/contracts";
...
type RpcUnaryMethod<TTag extends RpcTag> = RpcMethod<TTag> extends (input: any, options?: any) => Effect.Effect<infer TSuccess, any, any> ? (input: RpcInput<TTag>) => Promise<TSuccess> : never;
type RpcUnaryNoArgMethod<TTag extends RpcTag> = RpcMethod<TTag> extends (input: any, options?: any) => Effect.Effect<infer TSuccess, any, any> ? () => Promise<TSuccess> : never;
type RpcStreamMethod<TTag extends RpcTag> = RpcMethod<TTag> extends (input: any, options?: any) => Stream.Stream<infer TEvent, any, any> ? (listener: (event: TEvent) => void, options?: StreamSubscriptionOptions) => () => void : never;
...
export interface WsRpcClient {
    ...
    readonly git: {
        ...
        readonly onStatus: (input: RpcInput<typeof WS_METHODS.subscribeGitStatus>, listener: (status: GitStatusResult) => void, options?: StreamSubscriptionOptions) => () => void;
        readonly runStackedAction: (input: GitRunStackedActionInput, options?: GitRunStackedActionOptions) => Promise<GitRunStackedActionResult>;
        ...
        readonly suggestCommitMessage: (input: GitSuggestCommitMessageInput) => Promise<GitSuggestCommitMessageResult>;
    ...
    readonly server: {
        ...
        readonly updateSettings: (patch: ServerSettingsPatch) => ReturnType<RpcUnaryMethod<typeof WS_METHODS.serverUpdateSettings>>;
