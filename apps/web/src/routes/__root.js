...
import { SlowRpcAckToastCoordinator, WebSocketConnectionCoordinator, WebSocketConnectionSurface, } from "../components/WebSocketConnectionSurface";
import { Button } from "../components/ui/button";
import { AnchoredToastProvider, ToastProvider, toastManager } from "../components/ui/toast";
import { resolveAndPersistPreferredEditor } from "../editorPreferences";
import { readNativeApi } from "../nativeApi";
import { getServerConfigUpdatedNotification, startServerStateSync, useServerConfig, useServerConfigUpdatedSubscription, useServerWelcomeSubscription, } from "../rpc/serverState";
import { clearPromotedDraftThread, clearPromotedDraftThreads, useComposerDraftStore, } from "../composerDraftStore";
...
function RootRouteView() {
    if (!readNativeApi()) {
        return (_jsx("div", { className: "flex h-screen flex-col bg-background text-foreground", children: _jsx("div", { className: "flex flex-1 items-center justify-center", children: _jsxs("p", { className: "text-sm text-muted-foreground", children: ["Connecting to ", APP_DISPLAY_NAME, " server..."] }) }) }));
    }
    return (_jsx(ToastProvider, { children: _jsxs(AnchoredToastProvider, { children: [_jsx(ServerStateBootstrap, {}), _jsx(EventRouter, {}), _jsx(WebSocketConnectionCoordinator, {}), _jsx(SlowRpcAckToastCoordinator, {}), _jsx(WebSocketConnectionSurface, { children: _jsx(AppSidebarLayout, { children: _jsx(Outlet, {}) }) })] }) }));
}
function RootRouteErrorView({ error, reset }) {
    const message = errorMessage(error);
    const details = errorDetails(error);
    return (_jsxs("div", { className: "relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6", children: [_jsxs("div", { className: "pointer-events-none absolute inset-0 opacity-80", children: [_jsx("div", { className: "absolute inset-x-0 top-0 h-44 bg-[radial-gradient(44rem_16rem_at_top,color-mix(in_srgb,var(--color-red-500)_16%,transparent),transparent)]" }), _jsx("div", { className: "absolute inset-0 bg-[linear-gradient(145deg,color-mix(in_srgb,var(--background)_90%,var(--color-black))_0%,var(--background)_55%)]" })] }), _jsxs("section", { className: "relative w-full max-w-xl rounded-2xl border border-border/80 bg-card/90 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:p-8", children: [_jsx("p", { className: "text-[11px] font-semibold tracking-[0.18em] text-muted-foreground uppercase", children: APP_DISPLAY_NAME }), _jsx("h1", { className: "mt-3 text-2xl font-semibold tracking-tight sm:text-3xl", children: "Something went wrong." }), _jsx("p", { className: "mt-2 text-sm leading-relaxed text-muted-foreground", children: message }), _jsxs("div", { className: "mt-5 flex flex-wrap gap-2", children: [_jsx(Button, { size: "sm", onClick: () => reset(), children: "Try again" }), _jsx(Button, { size: "sm", variant: "outline", onClick: () => window.location.reload(), children: "Reload app" })] }), _jsxs("details", { className: "group mt-5 overflow-hidden rounded-lg border border-border/70 bg-background/55", children: [_jsxs("summary", { className: "cursor-pointer list-none px-3 py-2 text-xs font-medium text-muted-foreground", children: [_jsx("span", { className: "group-open:hidden", children: "Show error details" }), _jsx("span", { className: "hidden group-open:inline", children: "Hide error details" })] }), _jsx("pre", { className: "max-h-56 overflow-auto border-t border-border/70 bg-background/80 px-3 py-2 text-xs text-foreground/85", children: details })] })] })] }));
}
...
function coalesceOrchestrationUiEvents(events) {
    ...
    for (const event of events) {
        ...
        if (previous?.type === "thread.message-sent" &&
            ...
            coalesced[coalesced.length - 1] = {
                ...
                payload: {
                    ...
                    text: !event.payload.streaming && event.payload.text.length > 0
                        ? event.payload.text
...
function EventRouter() {
    ...
    const removeOrphanedTerminalStates = useTerminalStateStore((store) => store.removeOrphanedTerminalStates);
    ...
    const bootstrapFromSnapshotRef = useRef(async () => undefined);
    const serverConfig = useServerConfig();
    const handleWelcome = useEffectEvent((payload) => {
        if (!payload)
            return;
    ...
    const handleServerConfigUpdated = useEffectEvent((notification) => {
        if (!notification)
            return;
    ...
    useEffect(() => {
        ...
        const reconcileSnapshotDerivedState = () => {
            ...
            syncThreads(threads.map((thread) => ({
                id: thread.id,
                seedVisitedAt: thread.updatedAt ?? thread.createdAt,
            })));
            clearPromotedDraftThreads(threads.map((thread) => thread.id));
            const draftThreadIds = Object.keys(useComposerDraftStore.getState().draftThreadsByThreadId);
        ...
        const applyEventBatch = (events) => {
            ...
            const needsProjectUiSync = nextEvents.some((event) => event.type === "project.created" ||
            ...
            const needsThreadUiSync = nextEvents.some((event) => event.type === "thread.created" || event.type === "thread.deleted");
        ...
        const runReplayRecovery = async (reason) => {
            ...
            if (!disposed) {
                ...
                if (retryDecision.shouldRetry) {
                    ...
                    console.warn("[orchestration-recovery]", "Stopping replay recovery after no-progress retries.", {
                        state: recovery.getState(),
                    });
                }
            }
        };
        const runSnapshotRecovery = async (reason) => {
        ...
        const fallbackToSnapshotRecovery = async () => {
