...
const ENVIRONMENT_A = "environment-local";
const ENVIRONMENT_B = "environment-remote";
...
const {
  ...
  activeRunStackedActionDeferredRef: { current: createDeferredPromise() },
...
vi.mock("~/composerDraftStore", async () => {
  ...
  return {
    DraftId: {
      makeUnsafe: (value) => value,
    },
    useComposerDraftStore: Object.assign((selector) => selector(draftStoreState), {
...
function Harness() {
  ...
  return _jsxs(_Fragment, {
    children: [
      ...
      _jsx(GitActionsControl, { gitCwd: GIT_CWD, activeThreadRef: activeThreadRef }),
    ],
  });
}
