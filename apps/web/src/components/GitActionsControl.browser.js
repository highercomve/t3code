...
const { invalidateGitQueriesSpy, refreshGitStatusSpy, runStackedActionMutateAsyncSpy, setThreadBranchSpy, toastAddSpy, toastCloseSpy, toastPromiseSpy, toastUpdateSpy, } = vi.hoisted(() => ({
...
function Harness() {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", onClick: () => setActiveThreadId(THREAD_B), children: "Switch thread" }), _jsx(GitActionsControl, { gitCwd: GIT_CWD, activeThreadId: activeThreadId })] }));
}
describe("GitActionsControl thread-scoped progress toast", () => {
    ...
    it("keeps an in-flight git action toast pinned to the thread that started it", async () => {
        ...
        try {
            ...
            expect(toastUpdateSpy).toHaveBeenLastCalledWith("toast-1", expect.objectContaining({
                data: { threadId: THREAD_A },
                title: "Pushing...",
                type: "loading",
            }));
    ...
    it("debounces focus-driven git status refreshes", async () => {
        ...
        const host = document.createElement("div");
        document.body.append(host);
        const screen = await render(_jsx(GitActionsControl, { gitCwd: GIT_CWD, activeThreadId: THREAD_A }), {
            container: host,
        });
        try {
