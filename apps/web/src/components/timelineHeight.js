...
function estimateCharsPerLineForUser(timelineWidthPx) {
...
function estimateCharsPerLineForAssistant(timelineWidthPx) {
    ...
    return Math.max(MIN_ASSISTANT_CHARS_PER_LINE, Math.floor(textWidthPx / ASSISTANT_AVG_CHAR_WIDTH_PX));
}
export function estimateTimelineMessageHeight(message, layout = { timelineWidthPx: null }) {
    ...
    if (message.role === "user") {
        ...
        const renderedText = displayedUserMessage.contexts.length > 0
            ? [
