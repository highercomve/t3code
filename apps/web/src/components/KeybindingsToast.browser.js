...
const THREAD_ID = "thread-kb-toast-test";
...
}), http.get("*/attachments/:attachmentId", () => new HttpResponse(null, { status: 204 })), http.get("*/api/project-favicon", () => new HttpResponse(null, { status: 204 })));
function sendServerConfigUpdatedPush(issues) {
...
async function waitForComposerEditor() {
    return waitForElement(() => document.querySelector('[data-testid="composer-editor"]'), "App should render composer editor");
}
async function waitForToast(title, count = 1) {
    await vi.waitFor(() => {
...
async function waitForNoToast(title) {
    await vi.waitFor(() => {
...
async function mountApp() {
    ...
    const screen = await render(_jsx(AppAtomRegistryProvider, { children: _jsx(RouterProvider, { router: router }) }), { container: host });
...
describe("Keybindings update toast", () => {
    ...
    it("does not show a toast from the replayed cached value on subscribe", async () => {
        ...
        try {
            ...
            expect(titles.filter((t) => t === "Keybindings updated").length, "Replayed cached value should not produce a toast").toBe(0);
