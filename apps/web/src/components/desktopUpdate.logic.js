...
export function shouldShowArm64IntelBuildWarning(state) {
  return state?.hostArch === "arm64" && state.appArch === "x64";
}
export function isDesktopUpdateButtonDisabled(state) {
  return state?.status === "downloading";
}
export function getArm64IntelBuildWarningDescription(state) {
...
export function getDesktopUpdateButtonTooltip(state) {
  if (state.status === "available") {
