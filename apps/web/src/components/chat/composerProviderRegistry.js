...
import { isClaudeUltrathinkPrompt, normalizeClaudeModelOptionsWithCapabilities, normalizeCodexModelOptionsWithCapabilities, resolveEffort, } from "@t3tools/shared/model";
import { getProviderModelCapabilities, normalizeGeminiModelOptionsWithCapabilities, normalizeOpencodeModelOptionsWithCapabilities, } from "../../providerModels";
...
function getProviderStateFromCapabilities(input) {
    ...
    const ultrathinkActive = caps.promptInjectedEffortLevels.length > 0 && isClaudeUltrathinkPrompt(prompt);
    return {
        provider,
        promptEffort,
...
function makeCapabilitiesRegistryEntry(provider) {
    return {
        ...
        renderTraitsMenuContent: ({ threadRef, draftId, model, models, modelOptions, prompt, onPromptChange, }) => !hasComposerTraitsTarget({ threadRef, draftId }) ? null : (_jsx(TraitsMenuContent, { provider: provider, models: models, ...(threadRef ? { threadRef } : {}), ...(draftId ? { draftId } : {}), model: model, modelOptions: modelOptions, prompt: prompt, onPromptChange: onPromptChange })),
        renderTraitsPicker: ({ threadRef, draftId, model, models, modelOptions, prompt, onPromptChange, }) => !hasComposerTraitsTarget({ threadRef, draftId }) ? null : (_jsx(TraitsPicker, { provider: provider, models: models, ...(threadRef ? { threadRef } : {}), ...(draftId ? { draftId } : {}), model: model, modelOptions: modelOptions, prompt: prompt, onPromptChange: onPromptChange })),
    };
}
