            ...
            <div className="space-y-1.5">
              {thinkingEntries.length > 0 && (
                <div className="space-y-1">
                  {thinkingEntries.map((entry) => (
                    <ThinkingBlock key={`thinking:${entry.id}`} entry={entry} />
                  ))}
                </div>
              )}

              {contextTools.length > 0 && (
                <ContextToolGroup entries={contextTools} defaultOpen={contextTools.length <= 3} />
              )}
              ...
              {otherTools.length > 0 && (
                <div className="rounded-xl border border-border/45 bg-card/25 px-2 py-1.5">
                  ...
                  <div className="space-y-1">
                    {visibleOtherTools.map((workEntry) => (
                      <ToolCallBlock key={`tool:${workEntry.id}`} entry={workEntry} />
                    ))}
                  </div>
                </div>
              )}
