...
function effort(value, isDefault = false) {
  return {
    value,
    label: value,
...
describe("ProviderModelPicker", () => {
  ...
  it("opens provider submenus with a visible gap from the parent menu", async () => {
    ...
    try {
      ...
      const modelElement = Array.from(document.querySelectorAll('[role="menuitemradio"]')).find(
        (element) => element.textContent?.includes("GPT-5 Codex"),
      );
      if (!modelElement) {
