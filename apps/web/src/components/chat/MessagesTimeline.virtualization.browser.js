...
function MessagesTimelineBrowserHarness(props) {
  ...
  return _jsx("div", {
    ...
    children: _jsx(MessagesTimeline, {
      ...props,
...
async function measureTimelineRow(input) {
  const scrollContainer = await waitForElement(
    () => input.host.querySelector('[data-testid="messages-timeline-scroll-container"]'),
...
describe("MessagesTimeline virtualization harness", () => {
  ...
  it("preserves measured tail row heights when rows transition into virtualization", async () => {
    ...
    try {
      ...
      const scrollContainer = await waitForElement(
        () => mounted.host.querySelector('[data-testid="messages-timeline-scroll-container"]'),
        "Unable to find MessagesTimeline scroll container.",
      );
