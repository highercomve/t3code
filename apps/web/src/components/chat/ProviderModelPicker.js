...
export const ProviderModelPicker = memo(function ProviderModelPicker(props) {
    ...
    return (_jsxs(Menu, { open: isMenuOpen, onOpenChange: (open) => {
            ...
            setIsMenuOpen(open);
        }, children: [_jsx(MenuTrigger, { render: _jsx(Button, { size: "sm", variant: props.triggerVariant ?? "ghost", "data-chat-provider-model-picker": "true", className: cn("min-w-0 justify-start overflow-hidden whitespace-nowrap px-2 text-muted-foreground/70 hover:text-foreground/80 [&_svg]:mx-0", props.compact ? "max-w-42 shrink-0" : "max-w-48 shrink sm:max-w-56 sm:px-3", props.triggerClassName), disabled: props.disabled }), children: _jsxs("span", { className: cn("flex min-w-0 w-full box-border items-center gap-2 overflow-hidden", props.compact ? "max-w-36 sm:pl-1" : undefined), children: [_jsx(ProviderIcon, { "aria-hidden": "true", className: cn("size-4 shrink-0", providerIconClassName(activeProvider, "text-muted-foreground/70"), props.activeProviderIconClassName) }), _jsx("span", { className: "min-w-0 flex-1 truncate", children: selectedModelLabel }), _jsx(ChevronDownIcon, { "aria-hidden": "true", className: "size-3 shrink-0 opacity-60" })] }) }), _jsx(MenuPopup, { align: "start", children: props.lockedProvider !== null ? (_jsx(MenuGroup, { children: _jsx(MenuRadioGroup, { value: props.model, onValueChange: (value) => handleModelChange(props.lockedProvider, value), children: props.modelOptionsByProvider[props.lockedProvider].map((modelOption) => (_jsx(MenuRadioItem, { value: modelOption.slug, onClick: () => setIsMenuOpen(false), children: modelOption.name }, `${props.lockedProvider}:${modelOption.slug}`))) }) })) : (_jsxs(_Fragment, { children: [AVAILABLE_PROVIDER_OPTIONS.map((option) => {
