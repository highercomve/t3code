...
const {
  ...
  readEnvironmentApiMock: vi.fn((environmentId) => environmentApiById.get(environmentId)),
...
describe("TerminalViewport", () => {
  ...
  it("does not create a terminal when APIs are unavailable", async () => {
    ...
    const mounted = await mountTerminalViewport({
      threadRef: scopeThreadRef("environment-a", THREAD_ID),
    });
    try {
      await vi.waitFor(() => {
  ...
  it("reopens the terminal when the scoped thread reference changes", async () => {
    ...
    try {
      ...
      await mounted.rerender({
        threadRef: scopeThreadRef("environment-b", THREAD_ID),
      });
      await vi.waitFor(() => {
