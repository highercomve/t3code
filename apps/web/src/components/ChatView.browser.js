...
const ARCHIVED_SECONDARY_THREAD_ID = "thread-secondary-project-archived";
const PROJECT_ID = "project-1";
const SECOND_PROJECT_ID = "project-2";
...
function toShellSnapshot(snapshot) {
  return {
...
function updateThreadSessionInSnapshot(snapshot, threadId, session) {
  return {
    ...snapshot,
    snapshotSequence: snapshot.snapshotSequence + 1,
...
function sendShellThreadUpsert(threadId, options) {
...
function threadRefFor(threadId) {
  return scopeThreadRef(LOCAL_ENVIRONMENT_ID, threadId);
}
function threadKeyFor(threadId) {
  return scopedThreadKey(threadRefFor(threadId));
}
function composerDraftFor(target) {
  const { draftsByThreadKey } = useComposerDraftStore.getState();
  return draftsByThreadKey[target] ?? draftsByThreadKey[threadKeyFor(target)];
}
...
function draftThreadIdFor(draftId) {
...
function serverThreadPath(threadId) {
...
async function materializePromotedDraftThreadViaDomainEvent(threadId) {
...
async function startPromotedServerThreadViaDomainEvent(threadId) {
...
function createProjectlessSnapshot() {
  const snapshot = createSnapshotForTargetUser({
    targetMessageId: "msg-user-projectless-target",
...
function createSnapshotWithSecondaryProject(options) {
  const snapshot = createSnapshotForTargetUser({
    targetMessageId: "msg-user-secondary-project-target",
  ...
  const secondaryThreads = includeSecondaryThread
    ? [
        {
          id: "thread-secondary-project",
          ...
          checkpoints: [],
          session: {
            threadId: "thread-secondary-project",
            status: "ready",
            providerName: "codex",
            runtimeMode: "full-access",
            activeTurnId: null,
    ...
    : [];
  const archivedSecondaryThreads = includeArchivedSecondaryThread
    ? [
        {
...
async function pressComposerKey(key) {
...
async function pressComposerUndo() {
  const composerEditor = await waitForComposerEditor();
...
async function waitForComposerText(expectedText) {
  await vi.waitFor(
    () => {
...
async function selectAllComposerContent() {
  const composerEditor = await waitForComposerEditor();
  composerEditor.focus();
  const selection = window.getSelection();
  if (!selection) {
...
async function waitForComposerMenuItem(itemId) {
  return waitForElement(
    () => document.querySelector(`[data-composer-item-id="${itemId}"]`),
...
async function waitForSelectItemContainingText(text) {
  return waitForElement(
    () =>
      Array.from(document.querySelectorAll('[data-slot="select-item"]')).find((item) =>
...
async function expectComposerActionsContained() {
  const footer = await waitForElement(
    () => document.querySelector('[data-chat-composer-footer="true"]'),
    "Unable to find composer footer.",
  );
  const actions = await waitForElement(
    () => document.querySelector('[data-chat-composer-actions="right"]'),
...
async function openCommandPaletteFromTrigger() {
...
async function waitForCommandPaletteShortcutLabel() {
  await waitForElement(
...
async function mountChatView(options) {
  ...
  const screen = await render(
    _jsx(AppAtomRegistryProvider, { children: _jsx(RouterProvider, { router: router }) }),
    {
      container: host,
    },
  );
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("re-expands the bootstrap project using its scoped key", async () => {
    ...
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-bootstrap-project-expand",
  ...
  it("clears pending worktree overrides when switching empty server threads", async () => {
    const secondThreadId = "thread-browser-test-second";
    const snapshot = addThreadToSnapshot(createDraftOnlySnapshot(), THREAD_ID);
  ...
  it("surrounds selected plain text and preserves the inner selection for repeated wrapping", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-surround-basic",
  ...
  it("leaves collapsed-caret typing unchanged for surround symbols", async () => {
    ...
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-surround-collapsed",
  ...
  it("supports symmetric and backward-selection surrounds", async () => {
    ...
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-surround-backward",
  ...
  it("supports option-produced surround symbols like guillemets", async () => {
    ...
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-surround-guillemet",
  ...
  it("supports dead-key composition that resolves to another surround symbol without an extra undo step", async () => {
    ...
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-surround-dead-quote",
  ...
  it("surrounds text after a mention using the correct expanded offsets", async () => {
    ...
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-surround-after-mention",
  ...
  it("falls back to normal replacement when the selection includes a mention token", async () => {
    ...
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-surround-token",
  ...
  it("hides the archive action when the pointer leaves a thread row", async () => {
    ...
    try {
      ...
      const archiveButton = await waitForElement(
        () => document.querySelector(`[data-testid="thread-archive-${THREAD_ID}"]`),
  ...
  it("exposes the full thread title on the sidebar row tooltip", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-thread-tooltip-target",
  ...
  it("canonicalizes stale promoted draft routes to the server thread route", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-draft-hydration-race-test",
  ...
  it("renders the configurable shortcut and runs a command from the sidebar trigger", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-command-palette-shortcut-test",
  ...
  it("filters command palette results as the user types", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-command-palette-search-test",
  ...
  it("searches projects by path and opens the latest thread for that project", async () => {
    ...
    try {
      ...
      const nextPath = await waitForURL(
        mounted.router,
        (path) => path === serverThreadPath("thread-secondary-project"),
        "Route should have changed to the latest thread for the selected project.",
      );
      expect(nextPath).toBe(serverThreadPath("thread-secondary-project"));
      expect(
        useComposerDraftStore.getState().getDraftThread(threadRefFor("thread-secondary-project")),
      ).toBeNull();
    } finally {
      await mounted.cleanup();
    }
  ...
  it("keeps plan follow-up footer actions fused and aligned after a real resize", async () => {
    ...
    try {
      ...
      await waitForElement(
        () => document.querySelector('button[aria-label="Implementation actions"]'),
    ...
    try {
      ...
      await vi.waitFor(
        () => {
          ...
          const actions = document.querySelector('[data-chat-composer-actions="right"]');
  ...
  it("shows a tooltip with the skill description when hovering a skill pill", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-skill-tooltip-target",
    ...
    try {
      ...
      await waitForElement(
        () => document.querySelector('[data-composer-skill-chip="true"]'),
