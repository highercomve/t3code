...
import "../index.css";
import { EventId, ORCHESTRATION_WS_METHODS, WS_METHODS, DEFAULT_SERVER_SETTINGS, } from "@t3tools/contracts";
...
import { INLINE_TERMINAL_CONTEXT_PLACEHOLDER, removeInlineTerminalContextPlaceholder, } from "../lib/terminalContext";
...
function createSnapshotForTargetUser(options) {
    ...
    for (let index = 0; index < 22; index += 1) {
        ...
        const attachments = isTarget && (options.targetAttachmentCount ?? 0) > 0
...
function addThreadToSnapshot(snapshot, threadId) {
    return {
        ...snapshot,
...
function createThreadCreatedEvent(threadId, sequence) {
    return {
        sequence,
...
function sendOrchestrationDomainEvent(event) {
    rpcHarness.emitStreamValue(WS_METHODS.subscribeOrchestrationDomainEvents, event);
}
async function waitForWsClient() {
    await vi.waitFor(() => {
        expect(wsRequests.some((request) => request._tag === WS_METHODS.subscribeOrchestrationDomainEvents)).toBe(true);
...
async function promoteDraftThreadViaDomainEvent(threadId) {
    await waitForWsClient();
    fixture.snapshot = addThreadToSnapshot(fixture.snapshot, threadId);
    sendOrchestrationDomainEvent(createThreadCreatedEvent(threadId, fixture.snapshot.snapshotSequence));
    await vi.waitFor(() => {
        expect(useComposerDraftStore.getState().draftThreadsByThreadId[threadId]).toBeUndefined();
    }, { timeout: 8_000, interval: 16 });
}
function createDraftOnlySnapshot() {
    const snapshot = createSnapshotForTargetUser({
        targetMessageId: "msg-user-draft-target",
...
function withProjectScripts(snapshot, scripts) {
    return {
        ...snapshot,
        projects: snapshot.projects.map((project) => project.id === PROJECT_ID ? { ...project, scripts: Array.from(scripts) } : project),
    };
}
function setDraftThreadWithoutWorktree() {
...
function createSnapshotWithLongProposedPlan() {
    const snapshot = createSnapshotForTargetUser({
        targetMessageId: "msg-user-plan-target",
    ...
    return {
        ...snapshot,
        threads: snapshot.threads.map((thread) => thread.id === THREAD_ID
...
function createSnapshotWithPendingUserInput(options) {
    const snapshot = createSnapshotForTargetUser({
        targetMessageId: "msg-user-pending-input-target",
...
function createSnapshotWithPlanFollowUpPrompt() {
    const snapshot = createSnapshotForTargetUser({
        targetMessageId: "msg-user-plan-follow-up-target",
...
function resolveWsRpc(body) {
    ...
    if (tag === WS_METHODS.terminalOpen) {
        return {
            ...
            worktreePath: typeof body.worktreePath === "string"
                ? body.worktreePath
...
const worker = setupWorker(wsLink.addEventListener("connection", ({ client }) => {
...
}), http.get("*/attachments/:attachmentId", () => HttpResponse.text(ATTACHMENT_SVG, {
    headers: {
...
async function waitForProductionStyles() {
    await vi.waitFor(() => {
...
async function waitForElement(query, errorMessage) {
    let element = null;
    await vi.waitFor(() => {
        element = query();
...
async function waitForURL(router, predicate, errorMessage) {
    let pathname = "";
    await vi.waitFor(() => {
...
async function waitForComposerEditor() {
    return waitForElement(() => document.querySelector('[contenteditable="true"]'), "Unable to find composer editor.");
}
async function waitForComposerMenuItem(itemId) {
    return waitForElement(() => document.querySelector(`[data-composer-item-id="${itemId}"]`), `Unable to find composer menu item "${itemId}".`);
}
async function waitForSendButton() {
    return waitForElement(() => document.querySelector('button[aria-label="Send message"]'), "Unable to find send button.");
}
function findComposerProviderModelPicker() {
    return document.querySelector('[data-chat-provider-model-picker="true"]');
}
function findButtonByText(text) {
    return (Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.trim() === text) ?? null);
}
async function waitForButtonByText(text) {
...
async function waitForButtonContainingText(text) {
    return waitForElement(() => findButtonContainingText(text), `Unable to find button containing "${text}".`);
}
async function expectComposerActionsContained() {
    const footer = await waitForElement(() => document.querySelector('[data-chat-composer-footer="true"]'), "Unable to find composer footer.");
    const actions = await waitForElement(() => document.querySelector('[data-chat-composer-actions="right"]'), "Unable to find composer actions container.");
    await vi.waitFor(() => {
...
async function waitForInteractionModeButton(expectedLabel) {
    return waitForElement(() => Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.trim() === expectedLabel), `Unable to find ${expectedLabel} interaction mode button.`);
}
async function waitForServerConfigToApply() {
    await vi.waitFor(() => {
        expect(wsRequests.some((request) => request._tag === WS_METHODS.subscribeServerConfig)).toBe(true);
    }, { timeout: 8_000, interval: 16 });
    await waitForLayout();
}
function dispatchChatNewShortcut() {
...
async function triggerChatNewShortcutUntilPath(router, predicate, errorMessage) {
...
async function waitForNewThreadShortcutLabel() {
...
async function measureUserRow(options) {
    ...
    const scrollContainer = await waitForElement(() => host.querySelector("div.overflow-y-auto.overscroll-y-contain"), "Unable to find ChatView message scroll container.");
    let row = null;
    await vi.waitFor(async () => {
        scrollContainer.scrollTop = 0;
        scrollContainer.dispatchEvent(new Event("scroll"));
        await waitForLayout();
        row = host.querySelector(rowSelector);
    ...
    scrollContainer.dispatchEvent(new Event("scroll"));
    await nextFrame();
    const timelineRoot = row.closest('[data-timeline-root="true"]') ??
        host.querySelector('[data-timeline-root="true"]');
    ...
    await vi.waitFor(async () => {
        ...
        scrollContainer.dispatchEvent(new Event("scroll"));
        await nextFrame();
        const measuredRow = host.querySelector(rowSelector);
...
async function mountChatView(options) {
    ...
    const screen = await render(_jsx(AppAtomRegistryProvider, { children: _jsx(RouterProvider, { router: router }) }), {
        container: host,
    });
    await waitForWsClient();
    ...
    return {
        ...
        cleanup,
        measureUserRow: async (targetMessageId) => measureUserRow({ host, targetMessageId }),
...
describe("ChatView timeline estimator parity (full app)", () => {
    beforeAll(async () => {
        fixture = buildFixture(createSnapshotForTargetUser({
            targetMessageId: "msg-user-bootstrap",
    ...
    it.each(TEXT_VIEWPORT_MATRIX)("keeps long user message estimate close at the $name viewport", async (viewport) => {
        const userText = "x".repeat(3_200);
        const targetMessageId = `msg-user-target-long-${viewport.name}`;
        const mounted = await mountChatView({
            viewport,
        ...
        try {
            const { measuredRowHeightPx, timelineWidthMeasuredPx, renderedInVirtualizedRegion } = await mounted.measureUserRow(targetMessageId);
            expect(renderedInVirtualizedRegion).toBe(true);
            const estimatedHeightPx = estimateTimelineMessageHeight({ role: "user", text: userText, attachments: [] }, { timelineWidthPx: timelineWidthMeasuredPx });
            expect(Math.abs(measuredRowHeightPx - estimatedHeightPx)).toBeLessThanOrEqual(viewport.textTolerancePx);
        }
        finally {
            await mounted.cleanup();
        }
    ...
    it("tracks wrapping parity while resizing an existing ChatView across the viewport matrix", async () => {
        const userText = "x".repeat(3_200);
        const targetMessageId = "msg-user-target-resize";
        const mounted = await mountChatView({
        ...
        try {
            ...
            for (const viewport of TEXT_VIEWPORT_MATRIX) {
                ...
                const estimatedHeightPx = estimateTimelineMessageHeight({ role: "user", text: userText, attachments: [] }, { timelineWidthPx: measurement.timelineWidthMeasuredPx });
                expect(measurement.renderedInVirtualizedRegion).toBe(true);
                expect(Math.abs(measurement.measuredRowHeightPx - estimatedHeightPx)).toBeLessThanOrEqual(viewport.textTolerancePx);
                measurements.push({ ...measurement, viewport, estimatedHeightPx });
            }
            expect(new Set(measurements.map((measurement) => Math.round(measurement.timelineWidthMeasuredPx)))
                .size).toBeGreaterThanOrEqual(3);
            const byMeasuredWidth = measurements.toSorted((left, right) => left.timelineWidthMeasuredPx - right.timelineWidthMeasuredPx);
    ...
    it("tracks additional rendered wrapping when ChatView width narrows between desktop and mobile viewports", async () => {
        ...
        const targetMessageId = "msg-user-target-wrap";
        const snapshot = createSnapshotForTargetUser({
            targetMessageId,
            targetText: userText,
        });
        ...
        const estimatedDesktopPx = estimateTimelineMessageHeight({ role: "user", text: userText, attachments: [] }, { timelineWidthPx: desktopMeasurement.timelineWidthMeasuredPx });
        const estimatedMobilePx = estimateTimelineMessageHeight({ role: "user", text: userText, attachments: [] }, { timelineWidthPx: mobileMeasurement.timelineWidthMeasuredPx });
    ...
    it.each(ATTACHMENT_VIEWPORT_MATRIX)("keeps user attachment estimate close at the $name viewport", async (viewport) => {
        const targetMessageId = `msg-user-target-attachments-${viewport.name}`;
        ...
        try {
            ...
            expect(Math.abs(measuredRowHeightPx - estimatedHeightPx)).toBeLessThanOrEqual(viewport.attachmentTolerancePx);
        }
        finally {
            await mounted.cleanup();
        }
    ...
    it("opens the project cwd for draft threads without a worktree path", async () => {
        ...
        try {
            ...
            const openButton = await waitForElement(() => Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.trim() === "Open"), "Unable to find Open button.");
            await vi.waitFor(() => {
    ...
    it("does not leak a server worktree path into drawer runtime env when launch context clears it", async () => {
        const snapshot = createSnapshotForTargetUser({
            targetMessageId: "msg-user-launch-context-target",
    ...
    it("filters the open picker menu and opens VSCodium from the menu", async () => {
        ...
        try {
            await waitForServerConfigToApply();
            const menuButton = await waitForElement(() => document.querySelector('button[aria-label="Copy options"]'), "Unable to find Open picker button.");
            menuButton.click();
            await waitForElement(() => Array.from(document.querySelectorAll('[data-slot="menu-item"]')).find((item) => item.textContent?.includes("VS Code Insiders")) ?? null, "Unable to find VS Code Insiders menu item.");
            expect(Array.from(document.querySelectorAll('[data-slot="menu-item"]')).some((item) => item.textContent?.includes("Zed"))).toBe(false);
            const vscodiumItem = await waitForElement(() => Array.from(document.querySelectorAll('[data-slot="menu-item"]')).find((item) => item.textContent?.includes("VSCodium")) ?? null, "Unable to find VSCodium menu item.");
            vscodiumItem.click();
            await vi.waitFor(() => {
                const openRequest = wsRequests.find((request) => request._tag === WS_METHODS.shellOpenInEditor);
                expect(openRequest).toMatchObject({
    ...
    it("runs project scripts from local draft threads at the project cwd", async () => {
        ...
        try {
            const runButton = await waitForElement(() => Array.from(document.querySelectorAll("button")).find((button) => button.title === "Run Lint"), "Unable to find Run Lint button.");
            runButton.click();
            await vi.waitFor(() => {
                const openRequest = wsRequests.find((request) => request._tag === WS_METHODS.terminalOpen);
                expect(openRequest).toMatchObject({
    ...
    it("runs project scripts from worktree draft threads at the worktree cwd", async () => {
        ...
        try {
            const runButton = await waitForElement(() => Array.from(document.querySelectorAll("button")).find((button) => button.title === "Run Test"), "Unable to find Run Test button.");
            runButton.click();
            await vi.waitFor(() => {
                const openRequest = wsRequests.find((request) => request._tag === WS_METHODS.terminalOpen);
                expect(openRequest).toMatchObject({
    ...
    it("lets the server own setup after preparing a pull request worktree thread", async () => {
        ...
        try {
            const branchButton = await waitForElement(() => Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.trim() === "main"), "Unable to find branch selector button.");
            branchButton.click();
            const branchInput = await waitForElement(() => document.querySelector('input[placeholder="Search branches..."]'), "Unable to find branch search input.");
            branchInput.focus();
            await page.getByPlaceholder("Search branches...").fill("1359");
            const checkoutItem = await waitForElement(() => Array.from(document.querySelectorAll("span")).find((element) => element.textContent?.trim() === "Checkout Pull Request"), "Unable to find checkout pull request option.");
            checkoutItem.click();
            const worktreeButton = await waitForElement(() => Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.trim() === "Worktree"), "Unable to find Worktree button.");
            worktreeButton.click();
            await vi.waitFor(() => {
                const prepareRequest = wsRequests.find((request) => request._tag === WS_METHODS.gitPreparePullRequestThread);
            ...
            expect(wsRequests.some((request) => request._tag === WS_METHODS.terminalWrite && request.data === "bun install\r")).toBe(false);
        }
        finally {
            await mounted.cleanup();
        }
    ...
    it("sends bootstrap turn-starts and waits for server setup on first-send worktree drafts", async () => {
        ...
        try {
            ...
            sendButton.click();
            await vi.waitFor(() => {
                const dispatchRequest = wsRequests.find((request) => request._tag === ORCHESTRATION_WS_METHODS.dispatchCommand);
            ...
            expect(wsRequests.some((request) => request._tag === WS_METHODS.terminalWrite &&
                request.threadId === THREAD_ID &&
                request.data === "bun install\r")).toBe(false);
        }
        finally {
            await mounted.cleanup();
        }
    ...
    it("shows the send state once bootstrap dispatch is in flight", async () => {
        ...
        try {
            ...
            sendButton.click();
            await vi.waitFor(() => {
                expect(wsRequests.some((request) => request._tag === ORCHESTRATION_WS_METHODS.dispatchCommand)).toBe(true);
    ...
    it("toggles plan mode with Shift+Tab only while the composer is focused", async () => {
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-target-hotkey",
        ...
        try {
            ...
            composerEditor.dispatchEvent(new KeyboardEvent("keydown", {
                key: "Tab",
                shiftKey: true,
                bubbles: true,
                cancelable: true,
    ...
    it("keeps removed terminal context pills removed when a new one is added", async () => {
        ...
        useComposerDraftStore.getState().addTerminalContext(THREAD_ID, createTerminalContext({
            id: "ctx-removed",
        ...
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-terminal-pill-backspace",
    ...
    it("shows a pointer cursor for the running stop button", async () => {
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-stop-button-cursor",
                targetText: "stop button cursor target",
                sessionStatus: "running",
            }),
        });
        try {
            const stopButton = await waitForElement(() => document.querySelector('button[aria-label="Stop generation"]'), "Unable to find stop generation button.");
    ...
    it("hides the archive action when the pointer leaves a thread row", async () => {
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-archive-hover-test",
        ...
        try {
            ...
            const archiveButton = await waitForElement(() => document.querySelector(`[data-testid="thread-archive-${THREAD_ID}"]`), "Unable to find archive button.");
    ...
    it("shows the confirm archive action after clicking the archive button", async () => {
        localStorage.setItem("t3code:client-settings:v1", JSON.stringify({
            ...DEFAULT_CLIENT_SETTINGS,
        ...
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-archive-confirm-test",
    ...
    it("keeps the new thread selected after clicking the new-thread button", async () => {
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-new-thread-test",
        ...
        try {
            ...
            const newThreadPath = await waitForURL(mounted.router, (path) => UUID_ROUTE_RE.test(path), "Route should have changed to a new draft thread UUID.");
            const newThreadId = newThreadPath.slice(1);
            ...
            await waitForURL(mounted.router, (path) => path === newThreadPath, "New thread should remain selected after server thread promotion clears the draft.");
    ...
    it("snapshots sticky codex settings into a new draft thread", async () => {
        ...
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-sticky-codex-traits-test",
    ...
    it("hydrates the provider alongside a sticky claude model", async () => {
        ...
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-sticky-claude-model-test",
        ...
        try {
            ...
            await expect.element(newThreadButton).toBeInTheDocument();
            await newThreadButton.click();
            const newThreadPath = await waitForURL(mounted.router, (path) => UUID_ROUTE_RE.test(path), "Route should have changed to a new sticky claude draft thread UUID.");
            const newThreadId = newThreadPath.slice(1);
            expect(useComposerDraftStore.getState().draftsByThreadId[newThreadId]).toMatchObject({
                modelSelectionByProvider: {
                    claudeAgent: {
    ...
    it("falls back to defaults when no sticky composer settings exist", async () => {
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-default-codex-traits-test",
    ...
    it("prefers draft state over sticky composer settings and defaults", async () => {
        ...
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-draft-codex-traits-precedence-test",
        ...
        try {
            ...
            await expect.element(newThreadButton).toBeInTheDocument();
            await newThreadButton.click();
            const threadPath = await waitForURL(mounted.router, (path) => UUID_ROUTE_RE.test(path), "Route should have changed to a sticky draft thread UUID.");
            const threadId = threadPath.slice(1);
            ...
            await newThreadButton.click();
            await waitForURL(mounted.router, (path) => path === threadPath, "New-thread should reuse the existing project draft thread.");
            expect(useComposerDraftStore.getState().draftsByThreadId[threadId]).toMatchObject({
                modelSelectionByProvider: {
                    codex: {
                        provider: "codex",
    ...
    it("creates a new thread from the global chat.new shortcut", async () => {
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-chat-shortcut-test",
        ...
        try {
            ...
            composerEditor.focus();
            await waitForLayout();
            await triggerChatNewShortcutUntilPath(mounted.router, (path) => UUID_ROUTE_RE.test(path), "Route should have changed to a new draft thread UUID from the shortcut.");
        }
        finally {
            await mounted.cleanup();
        }
    ...
    it("creates a fresh draft after the previous draft thread is promoted", async () => {
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-promoted-draft-shortcut-test",
        ...
        try {
            ...
            await waitForServerConfigToApply();
            await newThreadButton.click();
            const promotedThreadPath = await waitForURL(mounted.router, (path) => UUID_ROUTE_RE.test(path), "Route should have changed to a promoted draft thread UUID.");
            const promotedThreadId = promotedThreadPath.slice(1);
            await promoteDraftThreadViaDomainEvent(promotedThreadId);
            const freshThreadPath = await triggerChatNewShortcutUntilPath(mounted.router, (path) => UUID_ROUTE_RE.test(path) && path !== promotedThreadPath, "Shortcut should create a fresh draft instead of reusing the promoted thread.");
    ...
    it("keeps long proposed plans lightweight until the user expands them", async () => {
        ...
        try {
            await waitForElement(() => Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.trim() === "Expand plan"), "Unable to find Expand plan button.");
            expect(document.body.textContent).not.toContain("deep hidden detail only after expand");
            const expandButton = await waitForElement(() => Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.trim() === "Expand plan"), "Unable to find Expand plan button.");
            expandButton.click();
            await vi.waitFor(() => {
    ...
    it("uses the active worktree path when saving a proposed plan to the workspace", async () => {
        ...
        try {
            const planActionsButton = await waitForElement(() => document.querySelector('button[aria-label="Plan actions"]'), "Unable to find proposed plan actions button.");
            planActionsButton.click();
            const saveToWorkspaceItem = await waitForElement(() => (Array.from(document.querySelectorAll('[data-slot="menu-item"]')).find((item) => item.textContent?.trim() === "Save to workspace") ?? null), 'Unable to find "Save to workspace" menu item.');
    ...
    it("does not trigger numeric option shortcuts while the composer is focused", async () => {
        ...
        try {
            ...
            expect(document.body.textContent).not.toContain("How aggressive should the imaginary plan be?");
    ...
    it("submits multi-select questionnaire answers as arrays", async () => {
        ...
        try {
            ...
            nextButton.click();
            await vi.waitFor(() => {
                expect(document.body.textContent).toContain("How aggressive should the imaginary plan be?");
            }, { timeout: 8_000, interval: 16 });
    ...
    it("keeps plan follow-up footer actions fused and aligned after a real resize", async () => {
        ...
        try {
            const footer = await waitForElement(() => document.querySelector('[data-chat-composer-footer="true"]'), "Unable to find composer footer.");
            const initialModelPicker = await waitForElement(findComposerProviderModelPicker, "Unable to find provider model picker.");
            const initialModelPickerOffset = initialModelPicker.getBoundingClientRect().left - footer.getBoundingClientRect().left;
            await waitForButtonByText("Implement");
            await waitForElement(() => document.querySelector('button[aria-label="Implementation actions"]'), "Unable to find implementation actions trigger.");
            ...
            const implementActionsButton = await waitForElement(() => document.querySelector('button[aria-label="Implementation actions"]'), "Unable to find implementation actions trigger.");
            await vi.waitFor(() => {
                ...
                const compactModelPickerOffset = compactModelPicker.getBoundingClientRect().left - footer.getBoundingClientRect().left;
    ...
    it("keeps the slash-command menu visible above the composer", async () => {
        const mounted = await mountChatView({
            viewport: DEFAULT_VIEWPORT,
            snapshot: createSnapshotForTargetUser({
                targetMessageId: "msg-user-command-menu-target",
        ...
        try {
            ...
            const composerForm = await waitForElement(() => document.querySelector('[data-chat-composer-form="true"]'), "Unable to find composer form.");
            await vi.waitFor(() => {
                const menuRect = menuItem.getBoundingClientRect();
                const composerRect = composerForm.getBoundingClientRect();
                const hitTarget = document.elementFromPoint(menuRect.left + menuRect.width / 2, menuRect.top + menuRect.height / 2);
