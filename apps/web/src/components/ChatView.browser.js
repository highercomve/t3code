...
async function measureUserRow(options) {
  ...
  const scrollContainer = await waitForElement(
    () => host.querySelector("div.overflow-y-auto.overscroll-y-contain"),
  ...
  await nextFrame();
  const timelineRoot =
    row.closest('[data-timeline-root="true"]') ?? host.querySelector('[data-timeline-root="true"]');
...
async function mountChatView(options) {
  ...
  const screen = await render(_jsx(RouterProvider, { router: router }), {
    container: host,
  });
  await waitForLayout();
  const cleanup = async () => {
