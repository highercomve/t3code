...
import "../../index.css";
import { AuthSessionId, DEFAULT_SERVER_SETTINGS, EnvironmentId } from "@t3tools/contracts";
...
const authAccessHarness = vi.hoisted(() => {
  ...
  return {
    ...
    emitEvent,
    emitPairingLinkUpserted(pairingLink) {
      emitEvent({
        version: 1,
        revision,
    ...
    emitClientUpserted(clientSession) {
      emitEvent({
        version: 1,
        revision,
        type: "clientUpserted",
...
vi.mock("../../environments/runtime", () => {
  const primaryConnection = {
    ...
    client: {
      server: {
        subscribeAuthAccess: (listener) => authAccessHarness.subscribe(listener),
      },
    },
  ...
  return {
    ...
    resolveEnvironmentHttpUrl: (_environmentId, path) =>
    ...
    useSavedEnvironmentRegistryStore: (selector) => selector({ byId: {} }),
    useSavedEnvironmentRuntimeStore: (selector) => selector({ byId: {} }),
  };
});
...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("hides owner pairing tools in browser-served loopback builds without remote exposure", async () => {
    ...
    mounted = await render(
      _jsx(AppAtomRegistryProvider, { children: _jsx(ConnectionsSettings, {}) }),
    );
  ...
  it("shows diagnostics inside About with a single logs-folder action", async () => {
    ...
    mounted = await render(
      _jsx(AppAtomRegistryProvider, { children: _jsx(GeneralSettingsPanel, {}) }),
    );
