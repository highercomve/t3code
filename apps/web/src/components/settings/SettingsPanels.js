...
import { ArchiveIcon, ArchiveX, ChevronDownIcon, InfoIcon, LoaderIcon, PlusIcon, RefreshCwIcon, Undo2Icon, XIcon, } from "lucide-react";
...
import { canCheckForUpdate, getDesktopUpdateButtonTooltip, getDesktopUpdateInstallConfirmationMessage, isDesktopUpdateButtonDisabled, resolveDesktopUpdateButtonAction, } from "../../components/desktopUpdate.logic";
...
import { MAX_CUSTOM_MODEL_LENGTH, buildModelSelection, getCustomModelOptionsByProvider, resolveAppModelSelectionState, } from "../../modelSelection";
...
import { useServerAvailableEditors, useServerKeybindingsConfigPath, useServerObservability, useServerProviders, } from "../../rpc/serverState";
const THEME_OPTIONS = [
    {
        value: "system",
        label: "System",
...
function ProviderLastChecked({ lastCheckedAt }) {
    ...
    return (_jsx("span", { className: "text-[11px] text-muted-foreground/60", children: lastCheckedRelative.suffix ? (_jsxs(_Fragment, { children: ["Checked ", _jsx("span", { className: "font-mono tabular-nums", children: lastCheckedRelative.value }), " ", lastCheckedRelative.suffix] })) : (_jsxs(_Fragment, { children: ["Checked ", lastCheckedRelative.value] })) }));
}
function SettingsSection({ title, icon, headerAction, children, }) {
    return (_jsxs("section", { className: "space-y-3", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("h2", { className: "flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground", children: [icon, title] }), headerAction] }), _jsx("div", { className: "relative overflow-hidden rounded-2xl border bg-card text-card-foreground shadow-xs/5 not-dark:bg-clip-padding before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] dark:before:shadow-[0_-1px_--theme(--color-white/6%)]", children: children })] }));
}
function SettingsRow({ title, description, status, resetAction, control, children, }) {
    return (_jsxs("div", { className: "border-t border-border px-4 py-4 first:border-t-0 sm:px-5", children: [_jsxs("div", { className: "flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between", children: [_jsxs("div", { className: "min-w-0 flex-1 space-y-1", children: [_jsxs("div", { className: "flex min-h-5 items-center gap-1.5", children: [_jsx("h3", { className: "text-sm font-medium text-foreground", children: title }), _jsx("span", { className: "inline-flex h-5 w-5 shrink-0 items-center justify-center", children: resetAction })] }), _jsx("p", { className: "text-xs text-muted-foreground", children: description }), status ? _jsx("div", { className: "pt-1 text-[11px] text-muted-foreground", children: status }) : null] }), control ? (_jsx("div", { className: "flex w-full shrink-0 items-center gap-2 sm:w-auto sm:justify-end", children: control })) : null] }), children] }));
}
...
function SettingsPageContainer({ children }) {
    return (_jsx("div", { className: "flex-1 overflow-y-auto p-6", children: _jsx("div", { className: "mx-auto flex w-full max-w-4xl flex-col gap-6", children: children }) }));
}
function AboutVersionTitle() {
    return (_jsxs("span", { className: "inline-flex items-center gap-2", children: [_jsx("span", { children: "Version" }), _jsx("code", { className: "text-[11px] font-medium text-muted-foreground", children: APP_VERSION })] }));
}
function AboutVersionSection() {
    ...
    const handleButtonClick = useCallback(() => {
        ...
        if (action === "install") {
            const confirmed = window.confirm(getDesktopUpdateInstallConfirmationMessage(updateState ?? { availableVersion: null, downloadedVersion: null }));
            if (!confirmed)
                return;
            void bridge
                .installUpdate()
    ...
    const buttonDisabled = action === "none"
    ...
    const description = action === "download" || action === "install"
        ? "Update available."
        : "Current version of the application.";
    return (_jsx(SettingsRow, { title: _jsx(AboutVersionTitle, {}), description: description, control: _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { render: _jsx(Button, { size: "xs", variant: action === "install" ? "default" : "outline", disabled: buttonDisabled, onClick: handleButtonClick, children: buttonLabel }) }), buttonTooltip ? _jsx(TooltipPopup, { children: buttonTooltip }) : null] }) }));
}
...
export function GeneralSettingsPanel() {
    ...
    const [openProviderDetails, setOpenProviderDetails] = useState({
        ...
        copilotAgent: Boolean(settings.providers.copilotAgent.binaryPath !==
    ...
    const isGitWritingModelDirty = !Equal.equals(settings.textGenerationModelSelection ?? null, DEFAULT_UNIFIED_SETTINGS.textGenerationModelSelection ?? null);
    const openInPreferredEditor = useCallback((target, path, failureMessage) => {
        if (!path)
            return;
    ...
    const lastCheckedAt = serverProviders.length > 0
        ? serverProviders.reduce((latest, provider) => (provider.checkedAt > latest ? provider.checkedAt : latest), serverProviders[0].checkedAt)
        : null;
    return (_jsxs(SettingsPageContainer, { children: [_jsxs(SettingsSection, { title: "General", children: [_jsx(SettingsRow, { title: "Theme", description: "Choose how T3 Code looks across the app.", resetAction: theme !== "system" ? (_jsx(SettingResetButton, { label: "theme", onClick: () => setTheme("system") })) : null, control: _jsxs(Select, { value: theme, onValueChange: (value) => {
                            ...
                            DEFAULT_UNIFIED_SETTINGS.enableAssistantStreaming ? (_jsx(SettingResetButton, { label: "assistant output", onClick: () => updateSettings({
                            ...
                            }) })) : null, control: _jsx(Switch, { checked: settings.enableAssistantStreaming, onCheckedChange: (checked) => updateSettings({ enableAssistantStreaming: Boolean(checked) }), "aria-label": "Stream assistant messages" }) }), _jsx(SettingsRow, { title: "New threads", description: "Pick the default workspace mode for newly created draft threads.", resetAction: settings.defaultThreadEnvMode !== DEFAULT_UNIFIED_SETTINGS.defaultThreadEnvMode ? (_jsx(SettingResetButton, { label: "new threads", onClick: () => updateSettings({
                                defaultThreadEnvMode: DEFAULT_UNIFIED_SETTINGS.defaultThreadEnvMode,
                            }) })) : null, control: _jsxs(Select, { value: settings.defaultThreadEnvMode, onValueChange: (value) => {
                                if (value === "local" || value === "worktree") {
                                    updateSettings({ defaultThreadEnvMode: value });
                                }
                            }, children: [_jsx(SelectTrigger, { className: "w-full sm:w-44", "aria-label": "Default thread mode", children: _jsx(SelectValue, { children: settings.defaultThreadEnvMode === "worktree" ? "New worktree" : "Local" }) }), _jsxs(SelectPopup, { align: "end", alignItemWithTrigger: false, children: [_jsx(SelectItem, { hideIndicator: true, value: "local", children: "Local" }), _jsx(SelectItem, { hideIndicator: true, value: "worktree", children: "New worktree" })] })] }) }), _jsx(SettingsRow, { title: "Archive confirmation", description: "Require a second click on the inline archive action before a thread is archived.", resetAction: settings.confirmThreadArchive !== DEFAULT_UNIFIED_SETTINGS.confirmThreadArchive ? (_jsx(SettingResetButton, { label: "archive confirmation", onClick: () => updateSettings({
                                confirmThreadArchive: DEFAULT_UNIFIED_SETTINGS.confirmThreadArchive,
                            }) })) : null, control: _jsx(Switch, { checked: settings.confirmThreadArchive, onCheckedChange: (checked) => updateSettings({ confirmThreadArchive: Boolean(checked) }), "aria-label": "Confirm thread archiving" }) }), _jsx(SettingsRow, { title: "Delete confirmation", description: "Ask before deleting a thread and its chat history.", resetAction: settings.confirmThreadDelete !== DEFAULT_UNIFIED_SETTINGS.confirmThreadDelete ? (_jsx(SettingResetButton, { label: "delete confirmation", onClick: () => updateSettings({
                                confirmThreadDelete: DEFAULT_UNIFIED_SETTINGS.confirmThreadDelete,
                            }) })) : null, control: _jsx(Switch, { checked: settings.confirmThreadDelete, onCheckedChange: (checked) => updateSettings({ confirmThreadDelete: Boolean(checked) }), "aria-label": "Confirm thread deletion" }) }), _jsx(SettingsRow, { title: "Text generation model", description: "Configure the model used for generated commit messages, PR titles, and similar Git text.", resetAction: isGitWritingModelDirty ? (_jsx(SettingResetButton, { label: "text generation model", onClick: () => updateSettings({
                                textGenerationModelSelection: DEFAULT_UNIFIED_SETTINGS.textGenerationModelSelection,
                                        ...
                                        updateSettings({
                                            textGenerationModelSelection: resolveAppModelSelectionState({
                                                ...settings,
                                                textGenerationModelSelection: buildModelSelection(textGenProvider, textGenModel, nextOptions ?? undefined),
                                            }, serverProviders),
                                        });
                                    } })] }) })] }), _jsx(SettingsSection, { title: "Providers", headerAction: _jsxs("div", { className: "flex items-center gap-1.5", children: [_jsx(ProviderLastChecked, { lastCheckedAt: lastCheckedAt }), _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { render: _jsx(Button, { size: "icon-xs", variant: "ghost", className: "size-5 rounded-sm p-0 text-muted-foreground hover:text-foreground", disabled: isRefreshingProviders, onClick: () => void refreshProviders(), "aria-label": "Refresh provider status", children: isRefreshingProviders ? (_jsx(LoaderIcon, { className: "size-3 animate-spin" })) : (_jsx(RefreshCwIcon, { className: "size-3" })) }) }), _jsx(TooltipPopup, { side: "top", children: "Refresh provider status" })] })] }), children: providerCards.map((providerCard) => {
                    ...
                    return (_jsxs("div", { className: "border-t border-border first:border-t-0", children: [_jsx("div", { className: "px-4 py-4 sm:px-5", children: _jsxs("div", { className: "flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between", children: [_jsxs("div", { className: "min-w-0 flex-1 space-y-1", children: [_jsxs("div", { className: "flex min-h-5 items-center gap-1.5", children: [_jsx("span", { className: cn("size-2 shrink-0 rounded-full", providerCard.statusStyle.dot) }), _jsx("h3", { className: "text-sm font-medium text-foreground", children: providerDisplayName }), providerCard.versionLabel ? (_jsx("code", { className: "text-xs text-muted-foreground", children: providerCard.versionLabel })) : null, _jsx("span", { className: "inline-flex h-5 w-5 shrink-0 items-center justify-center", children: providerCard.isDirty ? (_jsx(SettingResetButton, { label: `${providerDisplayName} provider settings`, onClick: () => {
                                                                ...
                                                                } })) : null })] }), _jsxs("p", { className: "text-xs text-muted-foreground", children: [providerCard.summary.headline, providerCard.summary.detail ? ` - ${providerCard.summary.detail}` : null] })] }), _jsxs("div", { className: "flex w-full shrink-0 items-center gap-2 sm:w-auto sm:justify-end", children: [_jsx(Button, { size: "sm", variant: "ghost", className: "h-7 px-2 text-xs text-muted-foreground hover:text-foreground", onClick: () => setOpenProviderDetails((existing) => ({
                                                        ...existing,
                                                        ...
                                                        const shouldClearModelSelection = isDisabling && textGenProvider === providerCard.provider;
                                                        updateSettings({
                                                            providers: {
                                                                ...settings.providers,
                ...
                }) }), _jsx(SettingsSection, { title: "Advanced", children: _jsx(SettingsRow, { title: "Keybindings", description: "Open the persisted `keybindings.json` file to edit advanced bindings directly.", status: _jsxs(_Fragment, { children: [_jsx("span", { className: "block break-all font-mono text-[11px] text-foreground", children: keybindingsConfigPath ?? "Resolving keybindings path..." }), openKeybindingsError ? (_jsx("span", { className: "mt-1 block text-destructive", children: openKeybindingsError })) : (_jsx("span", { className: "mt-1 block", children: "Opens in your preferred editor." }))] }), control: _jsx(Button, { size: "xs", variant: "outline", disabled: !keybindingsConfigPath || isOpeningKeybindings, onClick: openKeybindingsFile, children: isOpeningKeybindings ? "Opening..." : "Open file" }) }) }), _jsxs(SettingsSection, { title: "About", children: [isElectron ? (_jsx(AboutVersionSection, {})) : (_jsx(SettingsRow, { title: _jsx(AboutVersionTitle, {}), description: "Current version of the application." })), _jsx(SettingsRow, { title: "Diagnostics", description: diagnosticsDescription, status: _jsxs(_Fragment, { children: [_jsx("span", { className: "block break-all font-mono text-[11px] text-foreground", children: logsDirectoryPath ?? "Resolving logs directory..." }), openDiagnosticsError ? (_jsx("span", { className: "mt-1 block text-destructive", children: openDiagnosticsError })) : null] }), control: _jsx(Button, { size: "xs", variant: "outline", disabled: !logsDirectoryPath || isOpeningLogsDirectory, onClick: openLogsDirectory, children: isOpeningLogsDirectory ? "Opening..." : "Open logs folder" }) })] })] }));
}
export function ArchivedThreadsPanel() {
    ...
    const handleArchivedThreadContextMenu = useCallback(async (threadId, position) => {
        const api = readNativeApi();
        if (!api)
            return;
    ...
    return (_jsx(SettingsPageContainer, { children: archivedGroups.length === 0 ? (_jsx(SettingsSection, { title: "Archived threads", children: _jsxs(Empty, { className: "min-h-88", children: [_jsx(EmptyMedia, { variant: "icon", children: _jsx(ArchiveIcon, {}) }), _jsxs(EmptyHeader, { children: [_jsx(EmptyTitle, { children: "No archived threads" }), _jsx(EmptyDescription, { children: "Archived threads will appear here." })] })] }) })) : (archivedGroups.map(({ project, threads: projectThreads }) => (_jsx(SettingsSection, { title: project.name, icon: _jsx(ProjectFavicon, { cwd: project.cwd }), children: projectThreads.map((thread) => (_jsxs("div", { className: "flex items-center justify-between gap-3 border-t border-border px-4 py-3 first:border-t-0 sm:px-5", onContextMenu: (event) => {
                    event.preventDefault();
                    void handleArchivedThreadContextMenu(thread.id, {
                        x: event.clientX,
                        y: event.clientY,
                    });
                }, children: [_jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("h3", { className: "truncate text-sm font-medium text-foreground", children: thread.title }), _jsxs("p", { className: "text-xs text-muted-foreground", children: ["Archived ", formatRelativeTimeLabel(thread.archivedAt ?? thread.createdAt), " \u00b7 Created ", formatRelativeTimeLabel(thread.createdAt)] })] }), _jsxs(Button, { type: "button", variant: "outline", size: "sm", className: "h-7 shrink-0 cursor-pointer gap-1.5 px-2.5", onClick: () => void unarchiveThread(thread.id).catch((error) => {
                            toastManager.add({
                                type: "error",
                                title: "Failed to unarchive thread",
                                description: error instanceof Error ? error.message : "An error occurred.",
                            });
                        }), children: [_jsx(ArchiveX, { className: "size-3.5" }), _jsx("span", { children: "Unarchive" })] })] }, thread.id))) }, project.id)))) }));
}
