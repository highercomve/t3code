...
function tokenStartForCursor(text, cursor) {
  let index = cursor - 1;
...
export function expandCollapsedComposerCursor(text, cursorInput) {
...
function collapsedSegmentLength(segment) {
  if (segment.type === "text") {
    return segment.text.length;
  }
  return 1;
}
function clampCollapsedComposerCursorForSegments(segments, cursorInput) {
...
export function clampCollapsedComposerCursor(text, cursorInput) {
  return clampCollapsedComposerCursorForSegments(
    splitPromptIntoComposerSegments(text),
    cursorInput,
  );
}
export function collapseExpandedComposerCursor(text, cursorInput) {
...
export function isCollapsedCursorAdjacentToInlineToken(text, cursorInput, direction) {
  const segments = splitPromptIntoComposerSegments(text);
...
export function detectComposerTrigger(text, cursorInput) {
...
export function parseStandaloneComposerSlashCommand(text) {
...
export function replaceTextRange(text, rangeStart, rangeEnd, replacement) {
