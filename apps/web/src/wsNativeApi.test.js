import { CommandId, DEFAULT_SERVER_SETTINGS, EventId, ProjectId, ThreadId, } from "@t3tools/contracts";
...
const orchestrationEventListeners = new Set();
...
const rpcClientMock = {
    ...
    terminal: {
        ...
        restart: vi.fn(),
        close: vi.fn(),
        onEvent: vi.fn((listener) => registerListener(terminalEventListeners, listener)),
    },
    projects: {
        searchEntries: vi.fn(),
        writeFile: vi.fn(),
    ...
    orchestration: {
        ...
        replayEvents: vi.fn(),
        onDomainEvent: vi.fn((listener) => registerListener(orchestrationEventListeners, listener)),
    },
};
vi.mock("./wsRpcClient", () => {
    return {
...
function emitEvent(listeners, event) {
    for (const listener of listeners) {
        listener(event);
    }
}
function getWindowForTest() {
...
function makeDesktopBridge(overrides = {}) {
    return {
        getWsUrl: () => null,
...
describe("wsNativeApi", () => {
    ...
    it("forwards context menu metadata to the desktop bridge", async () => {
        ...
        const { createWsNativeApi } = await import("./wsNativeApi");
        const api = createWsNativeApi();
        const items = [{ id: "delete", label: "Delete" }];
