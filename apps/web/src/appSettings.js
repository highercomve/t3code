...
import { getDefaultModel, getModelOptions, normalizeModelSlug, resolveSelectableModel, } from "@t3tools/shared/model";
...
const withDefaults = (fallback) => (schema) => schema.pipe(Schema.withConstructorDefault(() => Option.some(fallback())), Schema.withDecodingDefault(() => fallback()));
...
export function getCustomModelOptionsByProvider(settings, dynamicModelsByProvider) {
    ...
    return {
        codex: getAppModelOptions("codex", customModelsByProvider.codex, undefined, dynamicModelsByProvider?.codex),
        gemini: getAppModelOptions("gemini", customModelsByProvider.gemini, undefined, dynamicModelsByProvider?.gemini),
        claudeAgent: getAppModelOptions("claudeAgent", customModelsByProvider.claudeAgent, undefined, dynamicModelsByProvider?.claudeAgent),
        opencode: getAppModelOptions("opencode", customModelsByProvider.opencode, undefined, dynamicModelsByProvider?.opencode),
    };
}
...
export function useAppSettings() {
    const [settings, setSettings] = useLocalStorage(APP_SETTINGS_STORAGE_KEY, DEFAULT_APP_SETTINGS, AppSettingsSchema);
