...
export function setPendingUserInputCustomAnswer(draft, customAnswer) {
    const selectedOptionLabels = customAnswer.trim().length > 0
        ? undefined
...
export function findFirstUnansweredPendingUserInputQuestionIndex(questions, draftAnswers) {
    const unansweredIndex = questions.findIndex((question) => !resolvePendingUserInputAnswer(question, draftAnswers[question.id]));
    return unansweredIndex === -1 ? Math.max(questions.length - 1, 0) : unansweredIndex;
}
export function derivePendingUserInputProgress(questions, draftAnswers, questionIndex) {
    const normalizedQuestionIndex = questions.length === 0 ? 0 : Math.max(0, Math.min(questionIndex, questions.length - 1));
