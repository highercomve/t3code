...
function modelSelection(provider, model, options) {
  return {
    provider,
    model,
    ...(options ? { options } : {}),
...
function providerModelOptions(options) {
  return options;
}
...
function threadKeyFor(threadId, environmentId = LEGACY_TEST_ENVIRONMENT_ID) {
  if (environmentId === LEGACY_TEST_ENVIRONMENT_ID) {
    return threadId;
  }
  return scopedThreadKey(scopeThreadRef(environmentId, threadId));
}
function draftFor(threadId, environmentId = LEGACY_TEST_ENVIRONMENT_ID) {
...
function createMockStorage() {
  const store = new Map();
  return {
    getItem: vi.fn((name) => store.get(name) ?? null),
