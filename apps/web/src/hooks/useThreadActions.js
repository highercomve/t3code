...
export function useThreadActions() {
    ...
    const deleteThread = useCallback(async (threadId, opts = {}) => {
        const api = readNativeApi();
        if (!api)
            return;
        ...
        const survivingThreads = deletedIds && deletedIds.size > 0
        ...
        const shouldDeleteWorktree = canDeleteWorktree &&
        ...
        const deletedThreadIds = opts.deletedThreadIds ?? new Set();
    ...
    const confirmAndDeleteThread = useCallback(async (threadId) => {
        const api = readNativeApi();
        if (!api)
            return;
        const thread = useStore.getState().threads.find((entry) => entry.id === threadId);
