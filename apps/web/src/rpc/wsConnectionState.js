...
const INITIAL_WS_CONNECTION_STATUS = Object.freeze({
  attemptCount: 0,
  closeCode: null,
  closeReason: null,
  connectedAt: null,
...
function updateWsConnectionStatus(updater) {
...
export function getWsConnectionUiState(status) {
...
export function recordWsConnectionAttempt(socketUrl) {
...
export function recordWsConnectionOpened() {
  return updateWsConnectionStatus((current) => ({
    ...current,
    closeCode: null,
    closeReason: null,
...
export function recordWsConnectionErrored(message) {
  return updateWsConnectionStatus((current) =>
...
export function setBrowserOnlineStatus(online) {
  return updateWsConnectionStatus((current) => ({
    ...current,
    online,
  }));
}
export function resetWsReconnectBackoff() {
  return updateWsConnectionStatus((current) => ({
    ...current,
    nextRetryAt: null,
    reconnectAttemptCount: 0,
...
export function exhaustWsReconnectIfStillWaiting(expectedNextRetryAt) {
  return updateWsConnectionStatus((current) => {
    if (
...
export function resetWsConnectionStateForTests() {
...
export function getWsReconnectDelayMsForRetry(retryIndex) {
...
function applyDisconnectState(current, updates) {
