...
function isStalePendingRequestFailureDetail(detail) {
    ...
    return (normalized.includes("stale pending approval request") ||
...
export function deriveWorkLogEntries(activities, latestTurnId) {
    ...
    return collapseDerivedWorkLogEntries(entries).map(({ activityKind: _activityKind, collapseKey: _collapseKey, ...entry }) => entry);
}
...
function trimMatchingOuterQuotes(value) {
    const trimmed = value.trim();
...
function splitExecutableAndRest(value) {
    const trimmed = value.trim();
    if (trimmed.length === 0) {
        return null;
    }
...
function findShellWrapperSpec(shell) {
    return SHELL_WRAPPER_SPECS.find((spec) => spec.executables.includes(shell));
}
function unwrapCommandRemainder(value, wrapperFlagPattern) {
...
function unwrapKnownShellCommandWrapper(value) {
...
function formatCommandArrayPart(value) {
...
function toRawToolCommand(value, normalizedCommand) {
    const formatted = formatCommandValue(value);
...
function extractWorkLogRequestKind(payload) {
    if (payload?.requestKind === "command" ||
        payload?.requestKind === "file-read" ||
        payload?.requestKind === "file-change") {
...
export function deriveCompletionDividerBeforeEntryId(timelineEntries, latestTurn) {
    ...
    if (latestTurn.assistantMessageId) {
        const exactMatch = timelineEntries.find((timelineEntry) => timelineEntry.kind === "message" &&
