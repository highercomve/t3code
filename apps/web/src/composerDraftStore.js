...
function normalizeProviderModelOptions(value, provider, legacy) {
  ...
  const copilotCandidate =
    candidate?.copilotAgent && typeof candidate.copilotAgent === "object"
  ...
  if (rawDraftThreadsByThreadId && typeof rawDraftThreadsByThreadId === "object") {
    for (const [threadKeyOrId, rawDraftThread] of Object.entries(rawDraftThreadsByThreadId)) {
      ...
      const promotedToRecord =
        promotedToCandidate && typeof promotedToCandidate === "object" ? promotedToCandidate : null;
      const promotedTo =
        promotedToRecord &&
...
const composerDraftStore = create()(
  persist(
    (setBase, get) => {
      const set = setBase;
      return {
