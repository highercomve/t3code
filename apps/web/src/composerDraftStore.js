import { CODEX_REASONING_EFFORT_OPTIONS, DEFAULT_MODEL_BY_PROVIDER, isProviderKind, ModelSelection, PROVIDER_KIND_SET, ProjectId, ProviderInteractionMode, ProviderKind, ProviderModelOptions, RuntimeMode, ThreadId, } from "@t3tools/contracts";
import { parseScopedProjectKey, parseScopedThreadKey, scopedProjectKey, scopeProjectRef, scopedThreadKey, scopeThreadRef, } from "@t3tools/client-runtime";
...
const ALL_PROVIDER_KINDS = [...PROVIDER_KIND_SET];
import { ensureInlineTerminalContextPlaceholders, normalizeTerminalContextText, } from "./lib/terminalContext";
...
const composerDebouncedStorage = createDebouncedStorage(typeof localStorage !== "undefined" ? localStorage : createMemoryStorage(), COMPOSER_PERSIST_DEBOUNCE_MS);
...
const PersistedDraftThreadState = Schema.Struct({
    ...
    promotedTo: Schema.optionalKey(Schema.NullOr(Schema.Struct({
        environmentId: Schema.String,
...
const EMPTY_COMPOSER_DRAFT_MODEL_STATE = Object.freeze({
    activeProvider: null,
...
function normalizeProviderModelOptions(value, provider, legacy) {
    ...
    const codexCandidate = candidate?.codex && typeof candidate.codex === "object"
        ? candidate.codex
        : null;
    const claudeCandidate = candidate?.claudeAgent && typeof candidate.claudeAgent === "object"
        ? candidate.claudeAgent
        : null;
    const codexReasoningEffort = codexCandidate?.reasoningEffort === "low" ||
        ...
        : provider === "codex" &&
            ...
            ? legacy.effort
            : undefined;
    const codexFastMode = codexCandidate?.fastMode === true
        ? true
        : codexCandidate?.fastMode === false
            ...
            : (provider === "codex" && legacy?.codexFastMode === true) ||
                ...
                ? true
                : undefined;
    const codex = codexReasoningEffort !== undefined || codexFastMode !== undefined
        ? {
            ...(codexReasoningEffort !== undefined ? { reasoningEffort: codexReasoningEffort } : {}),
            ...(codexFastMode !== undefined ? { fastMode: codexFastMode } : {}),
        }
        : undefined;
    const claudeThinking = claudeCandidate?.thinking === true
        ? true
        : claudeCandidate?.thinking === false
            ? false
            : undefined;
    const claudeEffort = claudeCandidate?.effort === "low" ||
    ...
    const claudeFastMode = claudeCandidate?.fastMode === true
        ...
        : claudeCandidate?.fastMode === false
            ? false
            : undefined;
    const claudeContextWindow = typeof claudeCandidate?.contextWindow === "string" && claudeCandidate.contextWindow.length > 0
        ? claudeCandidate.contextWindow
        : undefined;
    const claude = claudeThinking !== undefined ||
        ...
        : undefined;
    // ── Gemini options ────────────────────────────────────────────
    const geminiCandidate = candidate?.gemini && typeof candidate.gemini === "object"
        ? candidate.gemini
        : null;
    const geminiThinkingBudget = typeof geminiCandidate?.thinkingBudget === "number"
        ? geminiCandidate.thinkingBudget
        : undefined;
    const geminiReasoningEffort = geminiCandidate?.reasoningEffort === "low" ||
        ...
        : undefined;
    const gemini = geminiThinkingBudget !== undefined || geminiReasoningEffort !== undefined
        ? {
        ...
        : undefined;
    // ── OpenCode options ─────────────────────────────────────────
    const opencodeCandidate = candidate?.opencode && typeof candidate.opencode === "object"
        ? candidate.opencode
        : null;
    const opencodeReasoningEffort = opencodeCandidate?.reasoningEffort === "low" ||
        ...
        : undefined;
    const opencode = opencodeReasoningEffort !== undefined
        ? { reasoningEffort: opencodeReasoningEffort }
        : undefined;
    // ── Copilot options ──────────────────────────────────────────
    const copilotCandidate = candidate?.copilotAgent && typeof candidate.copilotAgent === "object"
        ? candidate.copilotAgent
        : null;
    const copilotReasoningEffort = copilotCandidate?.reasoningEffort === "low" ||
        ...
        : undefined;
    const copilotAgent = copilotReasoningEffort !== undefined ? { reasoningEffort: copilotReasoningEffort } : undefined;
...
function normalizeModelSelection(value, legacy) {
    ...
    const modelOptions = normalizeProviderModelOptions(candidate?.options ? { [provider]: candidate.options } : legacy?.modelOptions, provider, provider === "codex" ? legacy?.legacyCodex : undefined);
...
function legacyMergeModelSelectionIntoProviderModelOptions(modelSelection, currentModelOptions) {
    ...
    return legacyReplaceProviderModelOptions(normalizeProviderModelOptions(currentModelOptions), modelSelection.provider, modelSelection.options);
}
function legacyReplaceProviderModelOptions(currentModelOptions, provider, nextProviderOptions) {
    ...
    const normalizedNextProviderOptions = normalizeProviderModelOptions({ [provider]: nextProviderOptions }, provider);
    ...
    if (modelOptions) {
        for (const provider of ALL_PROVIDER_KINDS) {
            ...
            if (options && Object.keys(options).length > 0) {
                result[provider] = {
                    provider,
                    model: modelSelection?.provider === provider
...
export function deriveEffectiveComposerModelState(input) {
    const baseModel = normalizeModelSlug(input.threadModelSelection?.model ?? input.projectModelSelection?.model, input.selectedProvider) ?? getDefaultServerModel(input.providers, input.selectedProvider);
    ...
    const selectedModel = activeSelection?.model
        ...
        : baseModel;
    const modelOptions = modelSelectionByProviderToOptions(input.draft?.modelSelectionByProvider) ??
...
function projectDraftKey(projectRef) {
    return scopedProjectKey(projectRef);
}
function logicalProjectDraftKey(logicalProjectKey) {
 ...
 */
function composerTargetKey(target) {
 ...
 * incomplete identity.
 */
function normalizeLegacyComposerStorageKey(threadKeyOrId, options) {
    const parsedThreadRef = parseScopedThreadKey(threadKeyOrId);
    if (parsedThreadRef) {
        return composerTargetKey(parsedThreadRef);
    }
    if (options?.environmentId) {
        return composerTargetKey(scopeThreadRef(options.environmentId, threadKeyOrId));
    }
    return threadKeyOrId;
}
function composerThreadRefFromKey(threadKey) {
    return parseScopedThreadKey(threadKey);
}
function normalizeComposerTarget(state, target) {
    if (typeof target === "string") {
...
function resolveComposerDraftKey(state, target) {
    ...
    if (typeof normalizedTarget !== "string") {
        ...
        for (const [draftId, draftSession] of Object.entries(state.draftThreadsByThreadKey)) {
            if (draftSession.environmentId === normalizedTarget.environmentId &&
...
function resolveComposerThreadId(state, target) {
    const normalizedTarget = normalizeComposerTarget(state, target);
    if (!normalizedTarget) {
        return null;
    }
...
function getComposerDraftState(state, target) {
...
function isComposerThreadKeyInUse(mappings, threadKey) {
    return Object.values(mappings).includes(threadKey);
}
function toProjectDraftSession(draftId, draftSession) {
    return {
        draftId,
        ...draftSession,
    };
}
function createDraftThreadState(projectRef, threadId, logicalProjectKey, existingThread, options) {
    const projectChanged = existingThread !== undefined &&
        (existingThread.environmentId !== projectRef.environmentId ||
            existingThread.projectId !== projectRef.projectId);
    const nextWorktreePath = options?.worktreePath === undefined
        ? projectChanged
            ? null
            : (existingThread?.worktreePath ?? null)
        : (options.worktreePath ?? null);
    const nextBranch = options?.branch === undefined
        ? projectChanged
            ? null
...
function removeDraftThreadReferences(state, threadKey) {
    const nextLogicalMappings = Object.fromEntries(Object.entries(state.logicalProjectDraftThreadKeyByLogicalProjectKey).filter(([, draftThreadKey]) => draftThreadKey !== threadKey));
...
function normalizePersistedDraftThreads(rawDraftThreadsByThreadId, rawProjectDraftThreadIdByProjectKey) {
    const draftThreadsByThreadKey = {};
    const environmentIdByThreadId = new Map();
    if (rawProjectDraftThreadIdByProjectKey &&
        ...
        for (const [projectKey, threadId] of Object.entries(rawProjectDraftThreadIdByProjectKey)) {
            ...
            environmentIdByThreadId.set(threadId, projectRef.environmentId);
        }
    }
    if (rawDraftThreadsByThreadId && typeof rawDraftThreadsByThreadId === "object") {
        for (const [threadKeyOrId, rawDraftThread] of Object.entries(rawDraftThreadsByThreadId)) {
            ...
            const threadId = parsedThreadRef?.threadId ??
                (typeof candidateDraftThread.threadId === "string" &&
                    candidateDraftThread.threadId.length > 0
                    ? candidateDraftThread.threadId
                    : threadKeyOrId);
            const environmentId = parsedThreadRef?.environmentId ??
                (typeof candidateDraftThread.environmentId === "string" &&
                    candidateDraftThread.environmentId.length > 0
                    ? candidateDraftThread.environmentId
                    : environmentIdByThreadId.get(threadKeyOrId));
            ...
            const promotedToRecord = promotedToCandidate && typeof promotedToCandidate === "object"
                ? promotedToCandidate
                : null;
            const promotedTo = promotedToRecord &&
                ...
                ? scopeThreadRef(promotedToRecord.environmentId, promotedToRecord.threadId)
                : null;
            ...
            draftThreadsByThreadKey[threadKey] = {
                ...
                projectId: projectId,
                logicalProjectKey: typeof candidateDraftThread.logicalProjectKey === "string" &&
                    candidateDraftThread.logicalProjectKey.length > 0
                    ? candidateDraftThread.logicalProjectKey
                    : parsedThreadRef
                        ? projectDraftKey(scopeProjectRef(normalizedEnvironmentId, projectId))
                        : threadKeyOrId,
                ...
                interactionMode: candidateDraftThread.interactionMode === "plan" ||
    ...
    if (rawProjectDraftThreadIdByProjectKey &&
        typeof rawProjectDraftThreadIdByProjectKey === "object") {
        for (const [logicalProjectKey, threadKeyOrId] of Object.entries(rawProjectDraftThreadIdByProjectKey)) {
            if (typeof threadKeyOrId !== "string" || threadKeyOrId.length === 0) {
                continue;
            }
            ...
            if (!draftThreadsByThreadKey[threadKey]) {
                draftThreadsByThreadKey[threadKey] = {
                    threadId: parsedThreadRef?.threadId ?? threadKey,
                    environmentId: projectRef.environmentId,
                    projectId: projectRef.projectId,
                    logicalProjectKey,
            ...
            else if (draftThreadsByThreadKey[threadKey]?.projectId !== projectRef.projectId ||
...
function normalizePersistedDraftsByThreadId(rawDraftMap, draftThreadsByThreadKey) {
    ...
    for (const [threadKeyOrId, draftValue] of Object.entries(rawDraftMap)) {
        ...
        const runtimeMode = isRuntimeMode(draftCandidate.runtimeMode)
            ...
            : null;
        const interactionMode = draftCandidate.interactionMode === "plan" || draftCandidate.interactionMode === "default"
        ...
        if (draftCandidate.modelSelectionByProvider &&
            ...
            const normalizedModelOptions = normalizeProviderModelOptions(legacyDraftCandidate.modelOptions, undefined, legacyDraftCandidate) ?? null;
            ...
            const mergedModelOptions = legacyMergeModelSelectionIntoProviderModelOptions(normalizedModelSelection, normalizedModelOptions);
        ...
        const parsedThreadRef = parseScopedThreadKey(threadKeyOrId);
        const normalizedThreadKey = parsedThreadRef !== null
            ? normalizeLegacyComposerStorageKey(threadKeyOrId)
            : draftThreadsByThreadKey[threadKeyOrId] !== undefined
                ? threadKeyOrId
                : (() => {
                    const environmentId = environmentIdByThreadId.get(threadKeyOrId);
...
function migratePersistedComposerDraftStoreState(persistedState) {
    ...
    const rawProjectDraftThreadIdByProjectKey = candidate.logicalProjectDraftThreadKeyByLogicalProjectKey ??
    ...
    const nextStickyModelOptions = legacyMergeModelSelectionIntoProviderModelOptions(normalizedStickyModelSelection, stickyModelOptions);
    const stickyModelSelection = legacySyncModelSelectionOptions(normalizedStickyModelSelection, nextStickyModelOptions);
    const stickyModelSelectionByProvider = legacyToModelSelectionByProvider(stickyModelSelection, nextStickyModelOptions);
    const stickyActiveProvider = normalizeProviderKind(candidate.stickyProvider) ?? null;
    const { draftThreadsByThreadKey, logicalProjectDraftThreadKeyByLogicalProjectKey } = normalizePersistedDraftThreads(rawDraftThreadsByThreadId, rawProjectDraftThreadIdByProjectKey);
...
function partializeComposerDraftStoreState(state) {
    const persistedDraftsByThreadKey = {};
...
function normalizeCurrentPersistedComposerDraftStoreState(persistedState) {
    ...
    const { draftThreadsByThreadKey, logicalProjectDraftThreadKeyByLogicalProjectKey } = normalizePersistedDraftThreads(normalizedPersistedState.draftThreadsByThreadKey ??
    ...
    if (normalizedPersistedState.stickyModelSelectionByProvider &&
    ...
    return {
        draftsByThreadKey: normalizePersistedDraftsByThreadId(normalizedPersistedState.draftsByThreadKey ?? normalizedPersistedState.draftsByThreadId, draftThreadsByThreadKey),
        draftThreadsByThreadKey,
        logicalProjectDraftThreadKeyByLogicalProjectKey,
        stickyModelSelectionByProvider,
        stickyActiveProvider,
...
function readPersistedAttachmentIdsFromStorage(threadKey) {
...
function verifyPersistedAttachments(threadKey, attachments, set) {
    ...
    set((state) => {
        ...
        const persistedAttachments = attachments.filter((attachment) => imageIdSet.has(attachment.id) && persistedIdSet.has(attachment.id));
...
function hydratePersistedComposerImageAttachment(attachment) {
    ...
    try {
        ...
        if (!isBase64) {
            ...
            const inferredMimeType = header.startsWith("data:") && header.includes(";")
...
function toHydratedDraftThreadState(persistedDraftThread) {
    return {
        ...
        logicalProjectKey: persistedDraftThread.logicalProjectKey ??
            projectDraftKey(scopeProjectRef(persistedDraftThread.environmentId, persistedDraftThread.projectId)),
        ...
        promotedTo: persistedDraftThread.promotedTo
            ? scopeThreadRef(persistedDraftThread.promotedTo.environmentId, persistedDraftThread.promotedTo.threadId)
            : null,
    };
}
const composerDraftStore = create()(persist((setBase, get) => {
    const set = setBase;
    return {
        draftsByThreadKey: {},
        draftThreadsByThreadKey: {},
        ...
        getDraftSessionByProjectRef: (projectRef) => {
            for (const [draftId, draftThread] of Object.entries(get().draftThreadsByThreadKey)) {
                ...
                if (draftThread.projectId === projectRef.projectId &&
        ...
        getDraftSessionByRef: (threadRef) => {
            for (const draftSession of Object.values(get().draftThreadsByThreadKey)) {
                if (draftSession.environmentId === threadRef.environmentId &&
        ...
        listDraftThreadKeys: () => Object.values(get().draftThreadsByThreadKey).map((draftThread) => scopedThreadKey(scopeThreadRef(draftThread.environmentId, draftThread.threadId))),
        hasDraftThreadsInEnvironment: (environmentId) => Object.values(get().draftThreadsByThreadKey).some((draftThread) => draftThread.environmentId === environmentId),
        setLogicalProjectDraftThreadId: (logicalProjectKey, projectRef, draftId, options) => {
            ...
            set((state) => {
                ...
                const previousThreadKeyForLogicalProject = state.logicalProjectDraftThreadKeyByLogicalProjectKey[normalizedLogicalProjectKey];
                const nextDraftThread = createDraftThreadState(projectRef, options?.threadId ?? existingThread?.threadId ?? ThreadId.make(draftId), normalizedLogicalProjectKey, existingThread, options);
                ...
                const previousDraftThread = previousThreadKeyForLogicalProject === undefined
                    ? undefined
                    : nextDraftThreadsByThreadKey[previousThreadKeyForLogicalProject];
                if (previousThreadKeyForLogicalProject &&
                    previousThreadKeyForLogicalProject !== draftId &&
                    !isComposerThreadKeyInUse(nextLogicalProjectDraftThreadKeyByLogicalProjectKey, previousThreadKeyForLogicalProject) &&
        ...
        setDraftThreadContext: (threadRef, options) => {
            ...
            set((state) => {
                ...
                if (nextProjectRef.projectId.length === 0 ||
                    nextProjectRef.environmentId.length === 0) {
                    return state;
                }
                const projectChanged = nextProjectRef.environmentId !== existing.environmentId ||
                ...
                const isUnchanged = nextDraftThread.environmentId === existing.environmentId &&
        ...
        clearProjectDraftThreadId: (projectRef) => {
            set((state) => {
                const matchingThreadEntry = Object.entries(state.draftThreadsByThreadKey).find(([, draftThread]) => draftThread.projectId === projectRef.projectId &&
                    draftThread.environmentId === projectRef.environmentId);
        ...
        clearDraftThread: (threadRef) => {
            ...
            set((state) => {
                ...
                const hasLogicalProjectMapping = Object.values(state.logicalProjectDraftThreadKeyByLogicalProjectKey).includes(threadKey);
        ...
        setModelOptions: (threadRef, modelOptions) => {
            ...
            set((state) => {
                ...
                for (const provider of ALL_PROVIDER_KINDS) {
                    ...
                    if (!normalizedOpts || !(provider in normalizedOpts))
                        continue;
        ...
        setProviderModelOptions: (threadRef, provider, nextProviderOptions, options) => {
            ...
            const normalizedOpts = normalizeProviderModelOptions({ [normalizedProvider]: nextProviderOptions }, normalizedProvider);
            ...
            set((state) => {
                ...
                if (options?.persistSticky === true) {
                    ...
                    const stickyBase = nextStickyMap[normalizedProvider] ??
        ...
        setInteractionMode: (threadRef, interactionMode) => {
            ...
            const nextInteractionMode = interactionMode === "plan" || interactionMode === "default" ? interactionMode : null;
            set((state) => {
                const existing = state.draftsByThreadKey[threadKey];
        ...
        removeImage: (threadRef, imageId) => {
            ...
            set((state) => {
                ...
                const nextDraft = {
                    ...
                    persistedAttachments: current.persistedAttachments.filter((attachment) => attachment.id !== imageId),
                };
                const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
                if (shouldRemoveDraft(nextDraft)) {
                    delete nextDraftsByThreadKey[threadKey];
        ...
        insertTerminalContext: (threadRef, prompt, context, index) => {
            ...
            set((state) => {
                ...
                const dedupKey = terminalContextDedupKey(normalizedContext);
                if (existing.terminalContexts.some((entry) => entry.id === normalizedContext.id) ||
        ...
        addTerminalContext: (threadRef, context) => {
            ...
            get().addTerminalContexts(typeof threadRef === "string" ? DraftId.make(threadKey) : threadRef, [context]);
        },
            ...
            set((state) => {
                ...
                return {
                    draftsByThreadKey: {
                        ...
                        [threadKey]: {
                            ...existing,
                            prompt: ensureInlineTerminalContextPlaceholders(existing.prompt, existing.terminalContexts.length + acceptedContexts.length),
    ...
    merge: (persistedState, currentState) => {
        ...
        const draftsByThreadKey = Object.fromEntries(Object.entries(normalizedPersisted.draftsByThreadKey).map(([threadKey, draft]) => [
            threadKey,
            toHydratedThreadDraft(draft),
        ]));
        const draftThreadsByThreadKey = Object.fromEntries(Object.entries(normalizedPersisted.draftThreadsByThreadKey).map(([threadKey, draftThread]) => [threadKey, toHydratedDraftThreadState(draftThread)]));
        return {
            ...currentState,
            draftsByThreadKey,
            draftThreadsByThreadKey,
            logicalProjectDraftThreadKeyByLogicalProjectKey: normalizedPersisted.logicalProjectDraftThreadKeyByLogicalProjectKey,
...
export function useComposerThreadDraft(threadRef) {
    return useComposerDraftStore((state) => {
        return getComposerDraftState(state, threadRef) ?? EMPTY_THREAD_DRAFT;
    });
}
export function useComposerDraftModelState(threadRef) {
    return useComposerDraftStore(useShallow((state) => {
...
export function useEffectiveComposerModelState(input) {
    ...
    return useMemo(() => deriveEffectiveComposerModelState({
        draft,
 ...
 * threads at once.
 */
export function markPromotedDraftThread(threadId) {
    const store = useComposerDraftStore.getState();
    const draftThreadTargets = [];
...
export function finalizePromotedDraftThreadsByRef(serverThreadRefs) {
    for (const threadRef of serverThreadRefs) {
