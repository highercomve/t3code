...
export class BrowserWsRpcHarness {
    ...
    connect(client) {
        ...
        this.serverReady = Effect.runPromise(Scope.provide(this.scope)(RpcServer.makeNoSerialization(WsRpcGroup, this.makeServerOptions())).pipe(Effect.provide(this.makeLayer())));
    }
    async disconnect() {
        if (this.scope) {
    ...
    initializeStreamPubSubs() {
        this.streamPubSubs = new Map(Array.from(STREAM_METHODS, (method) => [method, Effect.runSync(PubSub.unbounded())]));
    }
    ...
    makeServerOptions() {
        return {
            onFromServer: (response) => Effect.sync(() => {
                if (!this.client) {
                    return;
                }
    ...
    handleStream(method, payload) {
        ...
        return Stream.fromIterable(this.getInitialStreamValues(request) ?? []).pipe(Stream.concat(Stream.fromPubSub(pubsub)));
    }
}
